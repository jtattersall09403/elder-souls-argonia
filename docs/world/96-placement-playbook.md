...
|---|---|---|
| Rollout 2026-09-09 | **Compile last.** Every build step stamps its output with the hash of what it read, so a hand edit to a blueprint after the pads are graded, or after a place is compiled, invalidates that step's receipt and the exporter refuses. Four "errors" that looked like a broken world were this. Fix, then run the chain — never the other way round | `grade_settlement_pads` receipt; `sourceBlueprintSha256`; the chain's `rederive_blueprints` -> `compile_settlement` order |
| Rollout 2026-09-09 | **Derived geometry drifts whenever the ground moves, and nothing re-derived it.** Ways are waypoints plus terrain routing; footprints, districts and doors derive from placed pieces; all of them read the shipped rasters. A re-carve makes every one stale, and it was a command a person had to remember after each rebuild — forgotten three times in one session | `worldgen/rederive_blueprints.py` as a chain stage; `rederive_terminals.py` for approach `viaUV`, terminal `entryUV` and gate yaw |
| Rollout 2026-09-09 | **A soft cost against a hard rule is two rules.** The street router charged x120 to cross a building while the validator forbade it outright, so a way whose detour was dearer went through the house and failed a later stage. Infinity is not the fix either — it makes every candidate cost infinite and A* reports success on the crossing | `street_router.PARCEL_PENALTY = 1e6`; a genuinely blocked corridor now survives to be named |
| Rollout 2026-09-09 | **A proxy for "the wall" is not the wall.** The door-facing check took the nearest footprint EDGE as the wall a door sits on. On a composite whose threshold sits 0.3 m from the hull centre, 0.3 m of re-derivation swings that "wall" 68 degrees and fails doors the kit itself placed. Where the pipeline mined a real entrance, the piece's own measured entrance is the authority | `blueprint.py` door-facing check; the mined `sideDeg` + yaw |
