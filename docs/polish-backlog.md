...
| Weather: cap-cloud banks seen against OPEN SKY — the aerial term only fogs surface fragments, so a mountain cloud bank is invisible where sky is behind it rather than terrain (round 4 dilation+noise fixed the "painted on" look, not this) | 8c round 4 (0032 §9) | a belt bank reads as cloud from every angle, including silhouetted against sky; needs the bank in the dome or a volumetric pass |
