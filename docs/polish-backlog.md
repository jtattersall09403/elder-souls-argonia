...
| Chain: the rebuild chain has FEEDBACK edges and never settles — `sculpt.corridor_and_anchor_mask` reads `apps/world-studio/public/province/routes.json` (`sculpt.py:105`) and so does `refine_province.rasterize_roads` (`refine_province.py:465`), and `reroute_majors` rewrites that file later in the same run. **Re-measured 2026-09-09 in an isolated worktree on a scratch vault: two `--force --from refine_province` runs on IDENTICAL sources moved 147 of the 1809 files the chain publishes**, including `routes.json`, `refined-height-ungraded-f32.npy`, both water surfaces, the pad grades and the postconditions. Starting below the sculpt does NOT avoid it — the refine edge alone is enough. This is why "36 files differ" was read as a fast-path divergence when it was the chain moving under both paths, and it is why no byte-identical acceptance test on the whole province is currently possible. `chain_stages.is_fresh` works around it, and `terrain-chain.sh` now refuses to re-run the frozen sculpt without `--allow-sculpt`, but neither is a fix | chain speed-up 2026-09-08; re-measured 2026-09-09 | either the stages that shape ground take the roads from a frozen snapshot the chain does not rewrite, or the route solves move ahead of the terrain; two forced runs on identical sources then produce identical bytes (measure with `scripts/chain-manifest.sh`) |
