...
| Repo tidiness: `tooling/world-generation/worldgen/__pycache__/*.pyc` is COMMITTED (11 files) — every test run dirties the tree and a `git stash`/`checkout` fights it | chain speed-up 2026-09-08 | the `__pycache__` directory is removed from the index and `.gitignore`d |
