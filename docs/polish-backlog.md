...
| **Water: the falls' SHAPE, now that their light is right** — the sheet reads as a cream rectangle with straight sides and a flat top standing in a thin white stream, where the reference is a narrow ribbon with torn edges that fray into spray. Three measured parts: the body flares two to three times wider than the river feeding its lip (compiled fall width, not shading); the side edges end on a hard silhouette instead of feathering; and the lit colour is warm ivory rather than neutral white. Plus: the falls kit takes no shadow at all (unlit shader, outside CSM), so a fall on a shaded gorge floor is lit as if in open sun — the 131 m gorge fall measures x2.10 of the water around it for that reason | water round 2 close-out, 2026-09-09; agent read of `artifacts/water-fall-25m.png` against `docs/research/rendering/reference/` | a fall is narrower than the stream feeding it, frays at both edges, reads neutral white, and darkens when it stands in shade |
