...
|---|---|---|
| **Regions: "raised hammock" covers 18 pixels — 0.1 ha of a 37 km² province, and holds zero places.** Measured 2026-09-09 by decoding `hydro-regions.png` through `worldgen.regions.REGION_CLASSES`: border mountains 704.5 ha, firm lowland 872.1, upland hills 514.8, rootland deep marsh 373.8, lake 331.1, fringe marsh 306.0, tropical jungle 297.6, interior swamp 176.1, mangrove 136.4, seasonal floodplain 47.3, coastal lagoon 17.3, deep river corridor 10.3, tidal delta 3.8, **raised hammock 0.1**. Not one of the 580 catalogue records sits in it (`plotFacts.regionClass` counts: firm lowland 189 … tidal delta 4, deep river corridor 1, raised hammock **0**). Dry palm islands on hummocks are a canonical Black Marsh landform and exactly the ground Argonian villages sit on, so a class the player can never stand in is a hole in the region grammar (00-core rule 7: region class changes movement, settlement forms, routes, encounters, materials, danger). Fixing it means re-running `compile_hydrology`, which re-solves the regions and invalidates the plot and the terrain — an owner call, not a quiet change. Also worth deciding whether tidal delta (3.8 ha) and deep river corridor (10.3 ha) are meant to be this thin | measured while re-fitting the vegetation ladder, 2026-09-09 | owner decides: either the hydrology produces raised hammocks at a size a player meets, or the class is retired from `REGION_CLASSES`, the vegetation ladder and the type recipes; no region class in the grammar covers less ground than the province can show |
...
| **Water: 2.72 km2 of the class raster is dry at EVERY season** — measured 2026-09-09 by splitting the class against the season the data itself publishes (`depth + 1.4 x seasonResponse`): of 31.40 km2 classed as water, 21.47 is wet in the dry season, 24.90 at the seasonal maximum, and 6.50 is dry all year — of which 1.50 sits within 0.5 m of flooding and **2.72 km2 stands more than 2 m above any water at any season**. That last part is the real defect; the rest is marsh and river margin that floods for months, which in a wet-and-dry-season province is the world working. The shipped meta now states the contract (class is a type label over the seasonal reach; wetness is depth plus the season lift), because its absence let a dock guard read the raster as a wetness mask | water round 2 close-out, 2026-09-09 | no classed cell stands more than 2 m above water at its own seasonal maximum |
| **Chain: a one-dock change costs a whole-province rebuild** — measured 2026-09-09: applying a 172 m x 12 m dock dredge re-ran `refine_province` (153 s), `compile_water` twice (2 x 68 s), every chunk (36 s), the web export (28 s), land cover (38 s) and scatter (26 s) = **461 s**, of which everything outside the dredge's own footprint is recomputed byte-identically. `chain_stages` fingerprints each STAGE but has no idea WHERE on the map a change lands. The dredge and the authored-waterway carve are the last things the carve does and touch a small known footprint, so they could patch the graded heightfield as a late stage and re-run only `compile_water` plus the chunk tiles they overlap. Phase 15 rollout pays this cost per packet | water round 2 close-out, 2026-09-09 | a local terrain edit rebuilds only the tiles it touches; a dock move costs seconds, not eight minutes |
