...
The Milestone 1 core already exceeds Oblivion's base journal count and approaches Skyrim's, while the mature target now matches Morrowind GOTY's *density* — our land is 1.5–2× Vvardenfell's, so matching quests-per-km² and POIs-per-km² (owner directive 2026-08-26) means exceeding its raw count; the world build carries the matching POI budget (the phases plan (docs/phases/README.md), Phase 15). Playtime estimates should be treated as ceilings, not marketing: Morrowind's base main quest is commonly completed in 25–30 hours; the 24-quest main line targets the same band.
