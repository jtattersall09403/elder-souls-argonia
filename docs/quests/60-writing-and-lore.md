...
| **Trying too hard** — any sentence written to land: the tag-line turn on the reader ("So are you."), the portentous closer ("a machine no living engineer can read"), the epigram ("The pace-count is a song"), the pithy predicate ("whose entire livelihood is that the falls exist") | two plain sentences; end the record on a fact | the general class behind the and-pair, the antithesis and the colon-reveal (owner 2026-09-04; style guide §2.8 has the examples and the rules) |
| Precision as emphasis: "exactly one", "exactly four", "only one", "one and only", "none of it", "none of them", "not one" | the plain number if it matters ("four expeditions", "this channel"), otherwise drop it | the successor tic to *the only* / *nobody* (owner 2026-09-04); *exactly N* and *only one* are hard linter rules |
| Repetition for effect ("the tree named … as the tree that stood") | say it once | a device, and a cheap one |
| A sentence or clause ending on a preposition ("the river Red Bramman escaped through", "dry enough to stack on") | "the river through which Red Bramman escaped"; "dry enough for stacking" | house grammar rule, all surfaces (owner 2026-09-04); hard linter rule |
| Rhetorical voice in a place record (address to the reader, a closer, a metaphor) | reference register: the voice of a wiki page about the place | place records are reference text, not dialogue (owner steer 2026-09-04, style guide §2.8) |
| "the total absence of X" as a `senses` closer ("the total absence of stone above knee height", "the total absence of any gate-bell", "the total absence of scavenger birds") | "no gate-bell anywhere"; or name what *is* heard | the sensory list ending on a grand negative; three in one region, always in the last slot (reviewer find, 2026-09-04) |
| Design or session voice in a quest premise or `questHooks.opportunity` ("so a forged grade is a quest and a bad one is a war", "every beat points at…", "the pattern is the point", "paid for with what the player can already see") | describe the situation in the world | the session-reference row was being dodged via premises and opportunity fields (round-5 reviewers, 2026-09-04) |
| The quest-shape tail on `questHooks.opportunity` ("…, which is a delve with a market's stock at stake", "…so the ledge is evidence before it is a fight", "…worth a whole quest with no faction attached") | end at the situation; the quest's shape belongs in the quest row | eleven in one region; the writer explaining the quest's design inside world prose (soft linter rule `design-voice`) |
| A zero relative clause ("ground the surrounding tribes leave alone", "the house he lives in", "the name they gave it") | write the pronoun, or turn it round: "ground that is left untouched by the surrounding tribes", "the house in which he lives" | the contact clause reads as speech and stacks a second subject on the noun (owner 2026-09-05; style guide §2.8 rule 8) |
| Soft modern idiom for a plain fact ("leave alone", "put up with", "make do", "keep to themselves") | the concrete verb: "do not cross", "tolerate", "have no", "the ground is untouched" | idiom that a fourth-era Argonian would not hold, and vaguer than the fact (style guide §2.8 rule 9) |
| The verbless epigram ("Short tide, short ledger.") | a sentence with a verb ("The tide was poor.") | a proverb the writer invented; an epigram in miniature |
| The same verb twice in one sentence by accident ("returns stolen goods, then refuses to return captives"; "the hold holds cargo") | a different verb | reads as the repetition device even when it was not meant |
| `vibe.mood` or `vibe.condition` as abstract noun + wry qualifying phrase ("cheerful extortion, priced by the hull"; "hospitable, on a scale a size too large"; "temporary by design, used for years") | one plain word, or a plain fact | the antithesis pair wearing a comma; label fields are labels |
| The same image in two prose fields of one record (`signatureFeature` "the mound is longer than the village that made it" + `hook` "a grave mound longer than the village that dug it") | say it once, in the field it belongs to | the player reads both panels; a repeated image reads as a device (soft linter rule `field-echo`) |
...
| A one-word label field punctuated as a sentence for weight ("Still.", "Patient.", "Cold.", "Heavy." in `mood`) | lower-case, no full stop: "still" | the full stop is the flourish; a label is not a sentence (hist-heartland review 2026-09-04) |
| The whole `questHooks.opportunity` set built to one formula: a fact copied from the record, then "so a quest can X" (34 of 51 in one region) | state the opportunity alone ("A quest can audit the repair ledger"); vary the opening across the set | the hook-copies-siteAdvantages row applies to opportunity fields too, and here it had become the set's single shape |
| The concealed-arrival formula in `vibe.approach` ("You are inside it before you see it"; "you are on the ground before you can see it"; "you are in the village before you have entered a building") | say what is visible, and from where: "There is no line of approach; the first houses are overhead when you reach them" | seven records in one region used the same reveal beat; it is a set-level tell, not a line (mercantile-coast review 2026-09-04) |
