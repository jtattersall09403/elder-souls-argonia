...
| C — parallel combat workstream (sandbox; feeds 10b) | in progress: measured weapon reach | Replace class reach estimates with per-weapon, per-attack measurements from the mounted hit volume and sourced attack motion. Inventory and AI will consume the shared result. Bow corrections remain delivered (0040, round 12). |
...
Weapon reach implementation is active; no owner reply is needed. Bow corrections remain available in the deployed sandbox (0040, round 12).
...
- **Combat:** measure weapon reach from production geometry and attacks, replace shared estimates, verify and deploy. Phase 10b remains the separate studio-integration step.
