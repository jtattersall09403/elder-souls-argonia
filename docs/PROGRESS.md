...
| 8b — water renderer and interaction | in progress | round-3 DEPLOYED 2026-08-28 (0025 = full log; research: tropical-fluvial-geomorphology): TIDE BUG fixed (data in PNG alpha channels premultiplied to zero by canvas decode — all rasters now RGB, probed); fluvial continuum terrain stage (worldgen/fluvial.py: Leopold–Maddock channel carving, levees/floodplains/backswamps, oxbows, wetland pools deepened, deltas; 6b noise lattice untouched); per-pixel blackwater/whitewater/clear water types (Sioli); CC0 beach/seabed/pebble textures ingested (35-material sets) + coast typing (mangrove/beach/salt-pan/cove/gravel). Full terrain→water chain rebuilt. Awaiting owner playtest round 3 |
