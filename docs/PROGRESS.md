...
| 8c — weather and atmosphere | in progress | **round 1 DEPLOYED 2026-08-29, awaiting owner playtest.** Continuing agents: the run-book at the top of decision [0032](decisions/0032-phase8c-weather-implementation-shape.md) is self-contained — "Continue phase 8C delivery" + feedback points is enough; it maps feedback→files, the validation loop and the hard-won rules. Built: deterministic synoptic machine (`packages/world-weather`) + local expression via `climate-weather.png`; envelope-safe dome clouds; rain streaks/ripples/wetness; 3 mist regimes in the one aerial term; wind→wave scale; env-query WeatherContact; studio `w=` force-state + weather presets. 10/10 browser probe scenarios green. Deferrals → polish backlog |
