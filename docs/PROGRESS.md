...
| Sound prep (Opus lead, 0087) | in progress: rounds 1–2 delivered 2026-09-24 — the pipeline from `Skyrim - Sounds.bsa` to Opus/WebM with sets read from Skyrim.esm and loop-safe beds, 391 first-consumer files (4.5 MB) in `packages/audio/files` ([0094](decisions/0094-audio-ships-as-opus-webm-sets-read-from-the-plugin-loops-crossfade-at-runtime.md)); the runtime: injected AudioManager, typed sound events, footstep and ambience contracts ([0095](decisions/0095-audio-runtime-is-an-injected-manager-over-an-engine-interface-fed-by-typed-sound-events.md)); round 3 (site budget, 12b docs) next | [brief](phases/lanes/sound-prep-lane.md) |
