...
- **8c round 2 playtest** — deployed 2026-08-29. Check: ① rain now visible
  (your downpour URL) and rippling water as far as you can see ② no
  lightning without a storm deck ③ night cloud cover reads (stars blotted
  in patches, moons dim/vanish behind cloud, moonlit edges) ④ clear days
  drift between empty sky and scattered cumulus, light dips when one
  crosses the sun ⑤ rain→downpour→squall→thunderstorm now a steep ladder
  with distinct looks (squall = dark wall upwind, lighter behind;
  thunderstorm = green-grey cast + racing scud) ⑥ clear-day summits no
  longer black-capped; fly up through the belt = white cloud, seen from
  below too ⑦ force dropdown has dawn mist / sea fog ⑧ storm seas rougher
  ⑨ water level follows the calendar (water season select in fly toolbar:
  auto/wet/dry; map view has a flood-wet layer) ⑩ fly starts at 400 m.
...
- **Workstream S — light sanity check (non-blocking)**: the round-2 rulings
  ...
  [owner round 2 §7](research/archive/workstream-s/stats-progression-owner-round2.md)
  and the [numbers packet](research/archive/workstream-s/stats-progression-numbers-packet.md)
  §4 (the whole-game runs) — both now archived as provenance.
