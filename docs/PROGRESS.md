...
| 8b — water renderer and interaction | in progress | round-6 DEPLOYED 2026-08-28 (0025 = full log): **mossy-cobble mystery closed** (bmv-v1 per-slot BMV_OVERRIDES silently kept the moss files — removed, TS beds/banks in both sets; dirt_cliff→trop_rocks); **rivers unified under one priority-flood physics** (bulges impossible by construction — owner's spot verified 0.00 m spread; centreline film vs rough ground = no dry gaps; carve ×1.3 = full channels); flow-aligned foam streaks; wetter heartlands incl. Archon jungle (compaction −0.6 m, denser rivulets); crates pushable (mass-unit scale, Phase 9 note); shallow-landing splashes. Awaiting owner playtest round 6 |
