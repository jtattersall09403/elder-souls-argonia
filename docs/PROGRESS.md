...
| C — parallel combat workstream (sandbox; feeds 10b) | in progress: combat completion audit | Click-triggered nocking, stable bow facing and sight calibration, fetch-arm constraint repair, shared 2× gravity and plain role labels implemented. Reversed forward runs replace rejected backward clips. Tests and typecheck pass; 11 bow scenes pass. Two backward-run stop checks exceed vertical-correction limits. See [0040](decisions/0040-animation-packs-and-combat-parallel-pass.md) round 11. |
...
- **Combat completion audit:** resolving the backward-stop support defect and auditing the full playtest request. No owner reply is needed during implementation. Existing limits and accepted embedding, full-draw appearance and sustained movement remain requirements.
