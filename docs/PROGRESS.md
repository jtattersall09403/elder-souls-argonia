...
| C — parallel combat workstream (sandbox; feeds 10b) | done: measured reach + bow/race corrections | All 35 melee weapons and 245 attacks measured from mounted geometry and sourced motion. Bow sight now follows the crosshair directly, locked aim centres once then detaches to mouse input, and stationary bow turns pivot around a planted sole. Ten playable bodies use distinct Skyrim FaceGen NPC records with the native FaceTint/detail material path. Root tests/typecheck and all 11 ranged/lock visual scenes pass. See [0040](decisions/0040-animation-packs-and-combat-parallel-pass.md) round 13 and [FaceGen pipeline research](research/combat-and-systems/skyrim-facegen-runtime-pipeline.md). |
