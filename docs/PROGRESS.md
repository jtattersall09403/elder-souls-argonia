...
| 8c — weather and atmosphere | in progress | **round 5 DEPLOYED 2026-08-30, awaiting owner playtest** (defect→fix log: decision [0032](decisions/0032-phase8c-weather-implementation-shape.md) Round 5): fog/mist/cap-cloud colour now DERIVED from the real sun/sky/moon (no authored fog colours; round 4's hand-dimming made all fog dark grey and far mountains near-black); dome fog march makes banks visible against open sky (closes the round-4 known limitation); cap-cloud slab off the map edge killed (belt-crossing-point sampling); rain descent ~2× in a 72 m soft-edged volume at 2.9× budget; fair/partly coverage retuned to real sky fractions; in-between-day hand-dimming removed (cloud-crossing sunOcclusion does it). 406 tests green incl. extended envelope proof; full probe suite ABORTED per the owner's 10-min visual-suite rule (compile/census/locality scenarios that ran were green). Round 4 history: All 7 round-3 feedback points fixed at root cause (defect→fix log: decision [0032](decisions/0032-phase8c-weather-implementation-shape.md) Round 4): rain drew BEHIND water (transparent, depthWrite-free, overpainted by the pass-3 water surface — now its own PRECIP_LAYER rendered after the water); rain moved off the time-scaled water clock onto real time at true terminal velocity, streak length derived from speed; less white; **GAME_TIME_SCALE = 30** (Morrowind's timescale) written into world-time and selectable in the studio; fair-weather coverage ladder clear→fair→partly→broken→overcast, rolled by the calendar with diurnal cumulus build; visibility integrated INTO weather (climate-vis G baseline × live regionHazeFactor — one call feeds the renderer and the published number); thunderstorm seas raised to squall parity; fog/mist/cap-cloud now lit and COLOURED by the sun (altitude dimming + sunset tint + directional forward-scatter fogSunLum), envelope proof extended; raster ClampToEdge stripes off the map edge fixed; cap cloud dilated + noise-broken. 22/22 probe scenarios + 406 tests green |
...
- **8c round 5 playtest** — deployed 2026-08-30. Check: ① **the round-4 dark
  grey is gone**: on a clear day, distant mountains fade into bright pale
  haze (not near-black), low mist reads white from above, and standing in
  mist by day is a bright white-out, not a darkening ② **sunset through
  mist**: at dusk the banks dim WITH the light and glow warm gold-rose on
  the side the sun is behind, cool grey on the other — the colours now come
  from the actual sun, so whatever the sky is doing, the mist matches it
  ③ **mountain cloud from outside**: on a rainy/overcast day, look at the
  massif from the lowlands — the cloud collar should now be visible even
  where open sky (a gap between peaks) is behind it, and there should be no
  flat white slab or straight-edged stripes over the sea off the map edge
  (your "image copy.png" spot: fly high over the SW corner and look around)
  ④ **rain**: streaks should fall visibly fast now (~2× round 4) at every
  intensity, and rain should surround you out to ~70 m fading softly into
  the distance — no hard wall of no-rain a few steps away ⑤ **the sky
  ladder**: force clear (nothing) → fair (a few fluffy cumulus, ~1/8 of the
  sky) → partly (~3/8, shadows crossing) → broken (as before) → overcast —
  and the in-between days should feel BRIGHT again, dimming only when a
  cloud actually crosses the sun ⑥ nothing regressed from round 4 (rain in
  front of water, game-speed rain rate, storm seas, night skies). Feedback
  as before: plain descriptions + screenshots with the time/weather panel
  visible. Defect→fix log: decision 0032 Round 5.
