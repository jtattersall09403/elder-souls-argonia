...
| Vegetation renderer — cells built once, LOD on the GPU, no rebuild on movement | in progress: rounds 1–2 delivered 2026-09-21 ([0082](decisions/0082-vegetation-cells-are-built-once-and-the-gpu-picks-the-rung.md)); performance rounds with the owner's walks under way, sixteen fixes committed locally, last c7343be (last: hard steps, hero plants swapped, tile mask, one cascade) (brief § Performance rounds); ground cover now in the lane; next: a fresh agent with "continue performance lane in line with my feedback below" plus the owner's HUD lines (brief § State at hand-off); closes when the jungle walk is smooth, then 16h | [brief](phases/lanes/vegetation-renderer-lane.md) |
