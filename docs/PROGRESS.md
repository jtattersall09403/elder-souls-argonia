...
| C — parallel combat workstream (sandbox; feeds 10b) | round 11 playtest pending; backward-stop gate open | Click-triggered nocking, stable bow facing and sight calibration, fetch-arm constraint repair, shared 2× gravity and plain role labels implemented. Reversed forward runs replace rejected backward clips. Tests and typecheck pass; 11 bow scenes pass. Two backward-run stop checks exceed vertical-correction limits. See [0040](decisions/0040-animation-packs-and-combat-parallel-pass.md) round 11. |
...
- **Combat round 11:** review the deployed bow fixes, reversed backward running and stopping. The backward-stop motion gate remains open: 11–12 cm vertical corrections exceed its 10.5 cm limit. No threshold was widened. Embedding, full-draw appearance and sustained bow movement remain accepted regression requirements.
