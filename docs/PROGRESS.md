...
| 8c — weather and atmosphere | in progress | **round 3 underway 2026-08-29** (owner feedback on round 2): fog must be LOCAL not province-wide (whiteout belt masked to the massif, summits above cloud, clear days clear; sea fog/dawn mist as local banks), rain genuinely visible (round-2 fix was not — root causes), region-baseline visibility rendered, sunset/sunrise cloud colouring, storm seas ~6× spectrum + wind-scaled shore surf. Defect→fix log: decision [0032](decisions/0032-phase8c-weather-implementation-shape.md). Run-book: top of 0032 |
...
- **8c round 3 playtest** — will replace this entry with the checklist when
  round 3 deploys (in progress; feedback being fixed at root cause).
