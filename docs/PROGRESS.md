...
([world/95-build-sequence.md](phases/README.md) §86). Read this file
...
| 11 — settlement/location system, exemplar-first (0034) | absorbed into 16 (2026-09-11) | Parts 0–6, Round A, assemblies, doors, promise rounds, B1 (buildings drawn 2026-09-09), the span trim and the rollout plot are recorded round by round in [0041](decisions/0041-phase11-settlement-decisions.md) and [research/archive/phase11-rounds/phase11-gap-plan.md](research/archive/phase11-rounds/phase11-gap-plan.md). The owner's walk of 2026-09-10 found the delivered settlements wrong at the runtime boundary ([audit](research/phase16/audit-settlements-delivered.md): yaw sign inverted, box colliders, anchoring to the terrain maximum, pads never shipped); the fixes and the exemplars now run as Phase 16 chunks 16h–16j |
| 12 — dungeon system, exemplar-first (0034; narrowed by 0061) | todo | dungeon families only (Xanmeer, cave/root/smuggler, underwater entrances, quest reservations, the stronghold interior). Building interiors and the interior load path are 16i's and are NOT rebuilt here |
...
| 10b — full portable-sandbox parity in studio (was 7b; moved 2026-08-25, decision 0017) | todo | Scene orchestration extraction (§53), inventory/equipment UI, enemies/targeting, bow, navmesh; combat-space probes then validate + freeze the 11/12 exemplar packets; **incl. fixes to shared combat internals** (owner 2026-08-29: good-enough, not perfect — specifics at kickoff). Probes run over the 16i exemplars, the 12 exemplars and every 15A packet; freezing is 15B's |
| T — text quality (parallel workstream, decision [0043](decisions/0043-text-quality-workstream.md)) | done | done 2026-09-03: docs/standards/text/ shelf, style guide, 8 culture registers, review process (0043). British spelling |
...
**16b round 2 is delivered, 2026-09-12** ([0060](decisions/0060-rivers-reach-the-coast-profiles-are-graded.md),
[ledger §9](research/phase16/16b-terrain-once-ledger.md)). After the owner's
walk of round 1: rivers run to the open sea again on the 16a network, taken
as given; river beds are graded, not stepped; a fall lands in a held pool
with a lip; the source's 2.97 m staircase is ramped before the mountains;
the Blackrose lake stands on a 1.6 m sill; the waterline is wet sand; ruling
2 fills 25 data holes and nothing else. **The 16a hydrograph is an approved
record** (`world/sources/hydrology/approved-bodies.json`): the ground is
shaped to hold it; every departure is listed for the owner (44 bodies
sit 1–5 m lower on the approved cliff work, 33 captured or joined to the sea
by carved channels, 54 unrealised and waived pending review, 13 new falls,
1,914 small new hollows drawn faint). The 2D map now opens on the frozen
ground with the proposed water and the old routes/waterways off. **Owner:**
walk the checklist in the handoff, then approve the departures as a
principle or send rows back; then `deliver 16c`.
...
owner will record themselves in [polish-backlog.md](phases/P-polish/backlog.md).
