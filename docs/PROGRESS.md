...
- **Phase 11 Part 7 Round A — siting and layout steer on the five exemplars**
  (2026-09-04). Read [research/phase11-part6-round-a.md](research/phase11-part6-round-a.md):
  it lists the five maps to open (PNGs in `tooling/world-generation/output/blueprint-maps/`
  on this machine), where each place ended up and why, and the four
  decisions per place that are yours (drowned-quarter depth and climbing
  rules in Lilmoth; the trunk-house ring in Nine-Trunks; pyramid versus
  building site in Mazzatun; pond depth and lethality at the Standing
  Charge; tree size and clearance at the Licensed Stage). Answer per place in
  plain words; Round B (massing) starts on the first place you approve.
  **Text (round 5):** every place panel, quest row and string was rewritten
  against your third list; read a dozen panels in the studio and the quest
  index and say whether the 'trying too hard' voice is gone. The old
  "With this death, a root is severed" line is now the reviewer's plain
  version, per your note.

- **Combat round-6 playtest (workstream C).** In the sandbox, not the world.
  Everything below answers your last list; the debug panel has two new
  switches so you can compare the big changes on and off in one session.

  **Arrows.** Every arrow — yours and the archer's — was flying tail first:
  the arrow model's head is at the opposite end from where the code assumed.
  Now they fly head first along a clean arc with the tail following, with no
  drag or spin physics, as you asked. Please check a few shots at long range
  and a couple lobbed high.

  **Archer.** It now holds a visible arrow on the string while drawing, tilts
  its upper body to aim, turns to face you *before* it looses (it will hold at
  full draw for up to a second waiting for the turn), and the arrow leaves
  from the bow along the way it is facing. Two things it still cannot do, both
  recorded as sourcing jobs with file paths: the bow string and limbs do not
  bend (the bow models were imported flat; Skyrim's bows carry their own small
  skeleton and draw animation), and there is no first-person rig — see below.

  **First-person bow view.** Standing still and turning the camera now turns
  your body with it, so the bow follows the crosshair; the arrow leaves from
  the nocked arrow, not from beside your head. I could not reproduce the
  "camera is on the left" point from the code alone — the eye is placed to
  the *right* of centre, toward the drawing hand, the way Skyrim does it. If
  it still looks wrong, tell me which side of the screen the bow hand, the
  arrow and the string should be on and I will move it. Skyrim's real
  first-person bow rig: you are right that it would be bows only, and all of
  it exists in vanilla (first-person skeleton, arm meshes for every race, the
  full first-person bow animation set). About a day of pipeline and runtime
  work; in the polish backlog with the paths.

  **Weapons held back to front.** Confirmed on the rig and fixed for every
  main-hand weapon at once (a half turn on the hand socket). Please glance at
  a war axe, a warhammer and a scimitar in hand.

  **Foot anchoring.** (1) Spinning attacks now carry you exactly where the
  feet say: a one-handed heavy takes you about 1.8 m to your right and a
  little back, and its blade lands off to your right — it will miss a
  locked-on target; you position for it. Enemies do the same when they swing
  from close range. "Attack movement from the feet" turns it all off for
  comparison. (2) The two-handed diagonal slide at the start and end of every
  swing was the standing pose itself sitting about 15 cm off-centre; fixed at
  source, so greatsword, hammer and battleaxe swings should start and end
  without the shuffle. (3) Locked-on strafing and back-walking now move at
  the speed their animations were made for (slow — under 1 m/s), so the
  planted foot is the anchor. A big feel change: "Locked-on speed follows the
  strafe clips" turns it off to compare. Crouch: its standing pose also sat
  5 cm off-centre, which shifted you every time you stopped and started;
  fixed the same way. Its speed has not changed in code since you liked it —
  if it still feels too fast, say so and I will measure it directly.

  **One check I could not make green, and a steer I need.** The automated
  "enemy parries your light attack" scene now fails: with the honest 8 cm
  side-step your sword reaches the parrying enemy's body but not its raised
  blade, and the game's parry only counts a clash with the blade, so neither
  a parry nor a hit happens. Two ways out, and it is a feel decision for you:
  (a) a parry catches by *timing* — anything that reaches the enemy while
  its parry is active is parried, the Dark Souls rule; or (b) keep it
  geometric and make the catch volume bigger. Say which. Every other scene in
  the suite passes.

  **Stabbing riposte.** On by default for one-handed swords and daggers;
  "Stabbing riposte (sword & dagger)" turns it off to compare with the old
  lunge. The greatsword riposte did not regress this time.

  **Your backstab question.** Only the two-handed axe and hammer were
  affected. When a backstab starts, the enemy plays a "being stabbed"
  reaction timed to *your* weapon's move; it was being timed to the enemy's
  own sword, whose move is far longer than an axe swing, so the moment it
  should react never came and the whole thing fizzled. With a sword the two
  happened to match. Fixed last round for all weapons in one place.
