...
| 8b — water renderer and interaction | in progress | round-6 DEPLOYED 2026-08-28, **awaiting owner playtest round 6** (not yet tested). **Fresh agents: the phase run-book — state, playtest checklist, rebuild chain, traps, open items — is at the TOP of decision 0025.** Round-6 highlights: mossy-cobble mystery closed (bmv-v1 BMV_OVERRIDES trap), rivers unified under one priority-flood physics (bulges impossible; centreline film = no dry gaps; carve ×1.3 = full channels), flow-aligned foam streaks, wetter heartlands incl. Archon jungle, pushable crates, shallow-landing splashes |
...
- **Owner visual playtest of 8b water ROUND 6** — deployed 2026-08-28,
  NOT yet owner-tested. A fresh agent continuing 8b starts at the
  **run-book at the top of decision
  [0025](decisions/0025-phase8b-water-implementation-shape.md)**: present
  the owner the round-6 checklist there, collect feedback, run defect→fix
  rounds. The owner also asked to re-review terrain feel (6b) once the
  water phase lands — combine with the closing pass.
