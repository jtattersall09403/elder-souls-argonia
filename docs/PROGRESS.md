...
| 8c — weather and atmosphere | in progress | **round 1 BUILT + probed, deploying 2026-08-29 — awaiting owner playtest** (checklist in *Waiting on user*). Deterministic synoptic machine (`packages/world-weather`, spells→slots, 18 tests) + local expression via `climate-weather.png`; dome cloud layers inside the light envelope; rain streaks/ripples/wetness; 3 mist regimes + weather fog in the one aerial term; wind→wave scale (CPU+GPU); overcast kills shadows; env-query WeatherContact; studio `w=` force-state + 4 weather presets. 10/10 browser probe scenarios green. Deferred w/ rationale → polish backlog (volumetrics, god rays, rain occlusion map, thunder audio→12b). Decision 0032 |
...
| S — stats, progression and character-systems **design** (parallel workstream, module 76; decision 0019) | in progress | **Round 1 CLOSED 2026-08-28** — all rulings incl. the F1 souls-from-skill-use levelling design in [owner-round1](research/stats-progression-owner-round1.md) (single source of truth) + decision 0031. Resume at §103.1 **step 5**: detail the decided design into module 76 → numbers packet (D0–D5 ladder) → tooling/stats-sim harness → owner round 2. Run-book: module 76 §103.1 (self-contained — "deliver workstream S" is enough). Skeleton is owner-chosen: **Morrowind chassis, Souls combat layer** (§103.0) — the work is the mapping exercise (seams, Skyrim-item→chassis taxonomy, keep/drop by sourceable movesets, completeness sweep). Docs + owner decisions only, no code; **exactly two batched owner rounds**. Must conclude before 10c |
...
- **Owner playtest of 8c round 1 (weather)** — deployed 2026-08-29.
  Suggested route: open the studio, use the *weather* selector in the time
  panel (or the new presets: storm noon / monsoon downpour / whiteout /
  squall front) and check ①clouds move and read at dawn/noon/night, ②rain
  falls, ground darkens+glosses and water ripples, ③storm skies feel dark
  and shadowless but playable, ④sea gets rougher in wind, ⑤"auto" weather
