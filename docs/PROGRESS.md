...
| Codespaces migration | in progress: owner setup done and vault snapshotted to R2 (76 parts, 45 GiB), `.devcontainer/` + `tooling/bootstrap/` + mod register delivered 2026-09-24; next: (1) prove the image on a first 4-core codespace (on-create, one kit build hash vs the VM, studio via the forwarded URL); (2) after the 16h part 1 commit: portability fixes in `terrain-chain.sh:126`, `chain-manifest.sh:18`, `memwatch.sh:31`; (3) after 16h part 1: miners' batch mode; (4) owner sets up the prebuild | [plan](research/infrastructure/codespaces-migration-plan.md) § Delivered, [bootstrap](../tooling/bootstrap/README.md) |
