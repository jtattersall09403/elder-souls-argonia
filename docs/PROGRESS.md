...
- **Phase 11 touchpoint ③ (round 3) — your third feedback list, all of it
  (2026-09-04; decision 0041 § Part 4 step 2, round 3 has the record).
  Deployed studio carries it. What to check, in order:
  1. **Map, `?cat=1&tracks=1&water=1`.** Open the filter panel: the new
     **quests** row — main quest, one chip per faction line, other
     questlines, minor side quests, plus "any quest" / "no quest" — shows
     where each line takes you. Click a dot: the **quests** section names
     each quest in plain words (code, title, which line, tier, and whether it
     owns the place or is merely built there). Travel-station destinations
     click through to their dot. Settlements say how many interiors they
     really have. Clicking a road, lane, track or channel now opens its panel;
     the painted roads no longer draw twice; the minor waterways draw.
  2. **Walk and fly:** the minimap now carries places, roads, lanes, tracks
     and channels, and fly mode has the same minimap. Minor roads are painted
     wider and no longer blur away — fly low over any track.
  3. **Text.** Every live record was rewritten by a separate reviewer per
     region against the new bans (comma-and, "never once", "the one thing",
     "nobody ever", the flat and-pair, *and* where *but* is meant), seeded
     from real Morrowind pages, and the linter reports **zero** hard hits
     province-wide (`python3 -m worldgen.lint_prose`, an `npm test` gate).
     Read a dozen panels and say whether it now reads right.
  4. **Consistency.** 240 audit findings resolved with judgment: some places
     now ASK for their ground (`terrainRequests` — the Black Eye gets its
     sinkhole at Part 6), some moved by typed siting (Ninefold now sits on
     its road; the boss ground is 61 m behind the Lost City, reached through
     it), the rest rewritten.
  5. **Hostility.** `world/sources/sites/hostility-frequency.md`: by travel
     you meet a fight every 100–140 m of route in D3–D5 (Morrowind ≈ 350 m);
     by area D3–D4 are under Morrowind's 15/km². Only the measured gap points
     were filled (six records). See the summary for the trade-off.
  6. **Quests as data.** `world/sources/quests/` holds 715 rows (525 new
     skeleton rows: every settlement at the floor of its Morrowind band);
     `docs/quests/index/` is generated from it. Skim `index/coverage.md` and
     one `index/local-<region>.md`.
  7. **Vault.** Boat mods registered; docks, watercraft and xanmeer-interior
     kits packaged; no House Dres set exists anywhere we own (Thorn keeps
     Hlaalu — steer needed, see summary).
  Decisions for you: (a) the total place ceiling (596) vs Morrowind's
  per-area hostility — add places or lean on Phase 13 roaming encounters;
  (b) Thorn: keep Hlaalu for its Dunmer quarter, or chase Tamriel Rebuilt's
  Dres set (permission + conversion); (c) the quest skeleton's total
  (715 rows, demand ladder 587+) against the 450–550 target — trim bands or
  accept a bigger province.
...
- **Combat round-5 playtest (workstream C).** In the sandbox, not the world.
  ...
  **Foot anchoring:** locked-on strafing and walking backwards now pace their
  steps to your real speed — much better, though at full stick the clips still
  can't quite keep up (they are authored slow); if it still bothers you, the
  options are a faster animation cap (can look scurried) or a slower locked-on
  walk (a feel change, your call). **The spinning attacks (greatsword second
  heavy) I have to be straight about:** I built the fix you described — body
  pivoting round the planted foot — and the measurements prove it cannot ship
  with these clips: the spin genuinely carries the body metres sideways, so
  the attack drifts off the enemy and misses (the automated fight checks
  failed outright with it on). Honest feet and an attack that tracks its
  target cannot both hold on a spinning clip. The real cure is finding
  replacement (non-spinning) versions of those few swings in the mod scene —
  logged in the polish backlog as a sourcing job.
