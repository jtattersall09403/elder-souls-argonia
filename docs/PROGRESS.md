...
- **Phase 11 touchpoint ③ (round 4) — your second list, plus the Part 5
  proposal** (2026-09-04; decision 0041 § Part 4 step 2, round 4). The
  deployed studio carries it. Check, in order:
  1. **Text.** Every place, quest row and text-catalogue string was rewritten
     against the new rule: *the only / the one / nobody / anyone / everyone /
     nothing / everything / always* are counted across the WHOLE body of text
     (13.5 → 1.3 per 1,000 words; "ask anyone" banned; no record over two),
     and each beat is tied to the place (a role, a name, a number, a
     direction, the goods) with the fix varied on purpose. The same-sentence-
     on-fourteen-records problem is now caught mechanically too. Read a dozen
     panels and the index (`docs/quests/index/`) and say if it reads local.
  2. **Consistency.** A place whose identity is a role in the network (a
     gate, a terminus, "the last X before Y", a lane station) now sits where
     that role exists or was cut: two cut, ~60 bound by typed siting. The Last
     Post is on the Blackwood Road's last cut before the closed western edge.
  3. **Minor roads** are cleared wider (paths cut through the growth; the
     herb layer is trodden to 8 %): fly low over a track or footpath.
  4. **Hostility.** Measured along the roads, lanes and cart tracks a player
     actually follows: a fight every ~140 m in D2–D3, ~180 m in D4, ~240 m in
     D5 (Morrowind ≈ 350 m); route-side clusters thinned by conversion or
     moved off-route; eleven hostile places added off-route in the measured
     D3–D4 gaps. By area D3 still sits at ~10/km² against Morrowind's 15; the
     province is dense enough that there are few real gaps left — the rest is
     Phase 13's roaming encounters.
  5. **Quest count**: 715 is the larger province, not creep — Morrowind's
     ~20 quests/km² on our ~37 km² of land is 550–740 (targets corrected).
  6. **Part 5 — pick the exemplars**: `docs/research/phase11-part5-exemplar-proposal.md`
     (Alten Corimont recommended as the city, Helstrom the alternative; five
     contrasting places). Part 6 starts on your picks.
  Decisions for you: (a) the exemplar set; (b) whether the remaining D3
  per-area shortfall should be closed with more places or left to Phase 13.
