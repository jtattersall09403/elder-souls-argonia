...
| 8b — water renderer and interaction | in progress | **round 7 DEPLOYED 2026-08-28, awaiting owner CLOSING playtest** (checklist at top of decision 0025). Round 7: slope rivers rebuilt as monotone long profiles (bed breaching + station-profile W bake; 100 % of centreline cells wet at all slopes, median 1.05 m on steep), 4-band legible flow speeds, de-crusted cascade foam, shader waterfall mode, shore surf system (fetch-gated asymmetric swash + shoaling swell + bore foam — the waveExposure gating bug muted all lapping in rounds 2–6), barcode root-caused (velocity × absolute time, now banned outright). 3 research docs. On PASS → done; leftovers → polish backlog (Phase P) |
...
- **Owner CLOSING playtest of 8b round 7** — deployed 2026-08-28.
  Checklist at the top of decision
  [0025](decisions/0025-phase8b-water-implementation-shape.md). This pass
  also covers the 6b terrain-feel re-review. On PASS, 8b flips to done and
  remaining cosmetics go to [polish-backlog.md](polish-backlog.md).
