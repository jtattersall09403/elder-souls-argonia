...
| Codespaces migration | in progress: owner setup done and vault snapshotted to R2 (76 parts, 45 GiB), `.devcontainer/` + `tooling/bootstrap/` + mod register delivered 2026-09-24; **first codespace proved 2026-09-25**: on-create + first-run.sh green (Wine prefix, pynifly, toolchain and cache tiers pulled, hashes match), owner walked map, fly, character and the combat sandbox via the forwarded 8081 URL, all good; `vault-pull.sh` pointer now in CLAUDE.md, the kit skills, the 16h brief and the asset docs; next: (1) one kit build hash vs the VM; (2) after the 16h part 1 commit: portability fixes in `terrain-chain.sh:126`, `chain-manifest.sh:18`, `memwatch.sh:31`; (3) after 16h part 1: miners' batch mode; (4) owner sets up the prebuild; (5) `ES_ASSET_PIPELINE_ROOT` has two meanings (vault root in `worldgen/vault.py:41`; kits root in `blueprint_footprints.py:69`, `blueprint_interiors.py:54`, `vegetationSolidity.test.ts:40`, `preflight.mjs:61`). Split it into two names; until then it must stay unset in a codespace (evidence: /tmp/codespaces-base-tier-audit.md, 0 of 1145 footprints when set) | [plan](research/infrastructure/codespaces-migration-plan.md) § Delivered, [bootstrap](../tooling/bootstrap/README.md) |
