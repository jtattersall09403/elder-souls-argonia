...
| C — parallel combat workstream (sandbox; feeds 10b) | in progress | Round 11: click-triggered nocking, downward aiming and drawing-arm repairs, backward-run sourcing, shared 2× arrow gravity default and plain sandbox role labels. Owner accepts round-10 embedding, full-draw appearance and sustained bow movement. Record: [0040](decisions/0040-animation-packs-and-combat-parallel-pass.md). |
...
- **Combat round 11:** implementing the latest playtest feedback for deployed review. No combat decision currently needs an owner reply. The accepted embedding, full-draw appearance and sustained movement remain regression requirements.
