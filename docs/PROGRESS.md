...
| 8b — water renderer and interaction | in progress | round-2 fixes DEPLOYED 2026-08-28 (0025 = full defect→fix log; new research: tropical-shoreline-materials, water-edges-and-shore-waves): water placement rebuilt on refined-grid priority-flood ("water finds its level", sail-cap probes), foam rebuilt as a system (contact line/lapping bands/whitecaps, turbidity-damped), speckle LOD, interactive ripple sim (Wallace/jeantimex), swash+wet-sand band (CPU/GPU/terrain lockstep), water-aware landcover beds (rebake_landcover.py), dpr cap + per-mode water draw distance. Next after playtest: CC0 beach/seabed texture ingestion per research Part D. Awaiting owner playtest round 3 |
