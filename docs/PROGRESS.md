...
| Placement workbench (Opus lead, 0087) | rounds 1–2 delivered 2026-09-24: a headless scene in which an agent places kit pieces over the frozen ground, measures every contact, renders and looks, and exports the poses as the blueprint record ([0097](decisions/0097-placement-is-authored-in-a-workbench-and-the-pose-record-is-the-output.md)); skill `placement-workbench`; round 3 (a second yard built only with it) in progress | [brief](phases/lanes/placement-workbench-lane.md) |
...
- **16k, the place loop ([0099](decisions/0099-places-are-built-in-a-loop-until-the-skill-is-proven.md)), two things to sign before slice 1's place is designed**
  (brief [16k](phases/16-foundation-and-places/16k-place-loop.md) § The type list, § The checklist):
  (1) the list of eight place types and the exit bar (each type comes out
  right twice in a row without you), with the checklist's Gate column;
  (2) the first real place: Claywater Station, a road-station village on
  the Gideon–Blackwood road with nothing to build first and no city
  nearby (Highwater is the second; Chasepoint dropped, too close to
  Soulrest).
