...
| Mining a shipped game's data for rules (placement, composition, grammars) | [world/95 §86.0b](world/95-build-sequence.md) + the readers `worldgen/esp_index.py` / `mine_placement.py` / `mine_groundcover.py` / `mine_micro_siting.py` / `mine_regions.py` + [what came out for vegetation](research/shipped-world-placement-rules.md) + [the `Skyrim.esm` cross-check](research/vanilla-skyrim-esm-placement-crosscheck.md) (Bethesda's own REGN/GRAS/placement numbers vs ours, with deltas to act on) + [micro-siting](research/mod-vegetation-micro-siting.md) and [the raw stats](../world/sources/placement/README.md) |
