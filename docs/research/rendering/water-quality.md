...
2. `python3 -m worldgen.compile_water` alone when only the compiler changed
   ...
   exempted cliff lips), `boundHitReaches` and `dryCoarseRiverCells` 0,
...
- Known, not ours: `province/settlements.json` is missing from the built studio (a concurrent settlement agent's `SettlementLayer` logs a JSON error at every site — the probe's page-error check fails until that file ships); `fall-20m-under` reports |still − ground − depth| 0.21 m (compiler data, 4.14 physical vs 4.34 compiled at 2174/268).
