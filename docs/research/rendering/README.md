...
| [building-placement-rendering-treatments.md](building-placement-rendering-treatments.md) | What a placed building needs to sit in the ground: contact shadow, edge foliage, a band of local ground texture. Read by `worldgen/vegetation_patches.py` and `game-core/vegetation/vegetationPatches.ts`. | live design input |
| [water-edges-and-shore-waves.md](water-edges-and-shore-waves.md) | Shore waves, wet sand and interactive ripples without a fluid sim, with the formulas. Read by `game-core/src/water/render/RippleSim.ts`, `waterMaterial.ts`. | history — written against the 8b Gerstner renderer; the live ledger is research/phase16/audit-water-runtime.md |
| [waterfalls-realtime.md](waterfalls-realtime.md) | How real-time games build falls and steep whitewater; §4.1 re-decided by 0064 (the vanilla kit's meshes are the fall). Read by `game-core/src/water/render/WaterfallKit*.ts`, `ChannelStrips.ts`. | live design input |
| [waterfall-mist-and-spray.md](waterfall-mist-and-spray.md) | The three kinds of waterfall mist, the spray and the splash (decision 0064): kit geometry, a per-fall ray-marched volume (why the weather's froxel cut does not apply), particle budgets. Read by `WaterfallMistVolume.ts`, `WaterCascadeSources.ts`. | live design input |
...
| [water-pro-greenheck-study.md](water-pro-greenheck-study.md) | How Dan Greenheck's Water Pro is built, why it looks good, and a ranked gap list against our water material. | technique reference only — its §6 plan carries no done marks; the transfer ledger with verdicts is research/phase16/audit-water-runtime.md §2 |
| [water-rendering-threejs.md](water-rendering-threejs.md) | The four reference water repos verified against their code, and the "why" behind decision 0025. | history (the "why" of 0025; runtime reshaped by 0046/0047) |
