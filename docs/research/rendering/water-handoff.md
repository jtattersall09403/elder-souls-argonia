...
The155 base constraints below still require resolution before final export.

## Immediate compiler work

Authoritative terrain: `tooling/world-generation/water-repair-inputs/bed-overlay.json`.
Matching solver cache: `/tmp/water-accepted-155-state.npz`; check the input
...
15,619 corrections remain. **155 channel constraints remain**, not completed
...
preservation passes; five focused routing tests pass. Current work considers
restoring obsolete bank cuts together with bed repairs and correcting lateral
sampling points within the authored footprint. Those proposals remain
unaccepted; use the durable inputs and155 cache above.
