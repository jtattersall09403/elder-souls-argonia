...
Matching solver cache: `/tmp/water-accepted-349-state.npz`; check the input
...
15,379 corrections remain. **349 channel constraints remain**, not completed
...
16 new failures and was rejected wholesale; next is a coupled proposal that
includes affected neighbours' fixed-head/bank constraints, not another tail loop.
