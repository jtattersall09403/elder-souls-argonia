...
|---|---|---|
| `on-create.sh` | apt `libarchive-tools` (bsdtar), `xvfb`, `wine` dependencies (i386 is not needed with the wow64 build), `rtree`/`libspatialindex`. `pip install -r tooling/world-generation/requirements-test.txt py7zr` plus a new `tooling/asset-pipeline/requirements.txt`. `npm ci`. Playwright chromium | apt, PyPI, npm |
