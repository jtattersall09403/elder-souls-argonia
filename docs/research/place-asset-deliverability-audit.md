...
| `shipyard` | deliverable | **hull-on-stocks CLOSED 2026-09-04**: the Cyrodiil ship resource ships the galleon's hull (`imperialship01base`) and its mast assembly as separate meshes, so a mastless hull raised on `works-v1` scaffold is authored geometry, not an invented piece. The hull is in `watercraft-v1`; the slipway itself is still read from dock and scaffold pieces. |
