...
**No neighbour worldspace ESPs exist in the vault**: `tamriel-worldspaces-118678` holds only `Argonia.esp`; the "126×126, −62..63" figure is our own `meta.json` `cellRange`, misattributed to Morrowind. What does exist is `mod-sources/all-tamriel-heightmap-573/…/TamrielBeta_10_2016_01_prepped.png` (20480 × 16384, 16-bit). ~~Registered by normalised cross-correlation: 8 full-res px per cell (~7.3 m/px), match at full-res (11960, 17752), r = 0.826; land-only fit ≈ 0.0152 m/unit − 50 m (r 0.63); the strips north, west and north-west are 100 % land.~~ **Wrong (16d, 2026-09-15, decision 0067):** that was a macro-shape false match on an island off Morrowind. The province is a **1:1 cut** of the map (32 px per cell, r = 0.9966, south-west sample at PNG row 3393, col 11788; the PNG is stored south-up). The map ends 16 km north, 6 km south, 21.5 km west and 8.5 km east of the border.

Delivered by 16d as decision 0067 describes (the map continued with a 6 km blend, the near ring as ordinary chunks, the sea over the apron below sea level); the superseded recommendation is not repeated here.
