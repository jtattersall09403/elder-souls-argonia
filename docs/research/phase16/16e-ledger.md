...
**Vault note (defect, mine, 2026-09-16; repaired the same day).** Re-running
`hydrology_graph derive` to redraw the overlays overwrote the vault's
`hydrology-graph-solution.npz` and `hydrology-graph-bodies.npz` (not in
git; read by every patch gate, `terrain_patches`, `terrain_preconditions`
and `compile_water`). The solution file was restored byte-identical from
the carve's copy (`province-refined/channels-pass1.npz`). The body rasters
were repaired without a refreeze (owner: never refreeze if it can be
avoided): starting from the re-derive, every frozen body absent from the
raster got its cells from the compiled water record at the graph's level
(59 bodies, 497k cells) and every body whose level had moved got the
graph's level back (68); 31 bodies under 500 m² have no compiled cells
and stay as the re-derive left them. Both water gates pass on it. Both
files are now in `freeze.json` (hashed by array content, so a re-save is
not drift) and `verify_freeze` checks them at the top of every chain run;
`hydrology_graph derive` refuses to run while the base is frozen without
`--refreeze`. The residual: the original raster may have differed from
this repair in ways nothing can measure now; the gates are the evidence.
