...
| Billboard mismatches | no card mapped by a wrong filename; 11 explicit borrows (three willows wear weeping-jungle silhouettes, the two composites a date palm); 115 flora species had NO card; the willows' own authored cards sample an aspen atlas rect | the 11 borrows removed: 10 bake from their own mesh (`bakeCard: "force"` on the three willows), the mangrove keeps its own authored card; 84 flora cards baked in all (118 of 159 species now carry a card) |
...
## 16. Round 3 (owner feedback of 2026-09-18, delivered the same day; decision 0072)

Every sentence of the owner's list, with the measured cause and what
shipped. Numbers from the headless probes against the local dev server
(`probe-local-16f.mjs`, `probe-local-walk.mjs`, SwiftShader at 800×450:
counts are exact, milliseconds are inflated by CPU contention on this VM and
are only ratios).

| Owner item | Measured cause | Shipped |
|---|---|---|
| "56 ha of tidal creek labelled ocean; queued for the next refreeze" — the owner never plans a refreeze; can it be ignored? | it costs the sea's class (tide, fetch, swell) and the ocean sea-bed bands on 56 ha of shallow tidal creek within 700 m of the open sea; nothing on land, no level, extent or route reads it | yes, ignored for good: the backlog row and decision 0071 now say so, with the cost written down so nobody reopens it |
| Jokerine's Seashells permission | a human page read | the owner confirmed "credit only" (2026-09-18); the sourcing-log row is CLOSED |
| Console: "Trying to use 16 texture units while this GPU supports only 16" at `WaterPipeline.tsx:304` (the water pass) | 17 samplers on the water material: 16d had noted it was at the limit and 16f's `uColourTex` was the 17th (three.js warns on the unit index, so "16" is the seventeenth) | algae rides `uSurfShore.a`, dark rides `uKlassTex.a`, packed at load (`packChannel`); the sampler is gone; 9 explicit samplers remain plus the environment map and the two cascades |
| Large rocks have disappeared entirely; trees have all disappeared | `Vegetation.tsx` never rebuilt: `kits/flora-province-v1.kit.json` came back from the dev server as `<!doctype html>` (Vite's SPA fallback; its cached public-file list predated the 10:44 kit rebuild, the server having started at 09:48) and the `Promise.all(...).catch(() => undefined)` swallowed the JSON error. Probe before the fix: `veg: null` after 160 s, the two kit GLBs and the index all HTTP 200, no bundle ever fetched. The sea-bed pieces vanished with them (same layer); the ring's own kit loaded, which is why grass survived | `es-fresh-public-files` middleware in `vite.config.ts`; the catch logs. After: jungle 25 chunks, 5,596 instances, 266 draws; Rockpark 20 chunks, 2,251 instances |
| Sensible LOD/distance approach for water? | already there: a grid dense inside `uniformRadius` with an exponential fringe, `esDetFade` (detail normals, exp(−d/100 m)), `esFarFade`, flow fade 150–400 m, rain rings ~100 m, crest fade 1.2–2.4 km, horizon blend 4–12 km; but SSR marched 18 steps to 1,200 m (fade 800–1,200) | SSR fades 260–420 m (`SSR_FADE_START_M/END_M`); nothing above 16f re-run; no water data touched |
| Plants/ground cover still in ordered straight rows | the ring's `hash32` had no finaliser: for consecutive salts `(uz − ux) mod 1` piled 85 % of its mass in two of twenty bins (7,527 and 10,115 against a flat 2,000), so every candidate lay on one of two diagonals per cell. The scatter compiler is SplitMix64 and clean. The Python twin's rows test drew `np.random`, so it could not see it | `fmix32` on both hashes (`ringHash.ts`, `groundcover_ring.py`); folded histogram now flat within 3 %; the twin draws the runtime's streams and `test_no_species_stands_in_rows` is red on the old hash (shown) |
| Ground cover fades OUT as you get closer, then everything reappears at once with a stutter; some types visible from a short distance then vanish when close | one tier per instance at rebuild: the MID card's band starts at 0.4 r, so walking inside 0.4 r − 4 m dithered it to nothing with no NEAR copy until the next rebuild, which only happened on a 16 m tile crossing and then regenerated a whole row of tiles (6.3 s for 350 tiles cold on this VM: ~18 ms a tile) and re-composed all 45k matrices with `setColorAt` (50 ms) — the stutter | tile-band membership with a 14 m overlap (decision 0072 §2); rebuilds every 8 m; generation in `useFrame` within 5 ms a frame (14 ms cold), nearest first; matrices and colours composed at generation and block-copied at fill (fill 1.5–15 ms measured, 50 before); spheres from tile extents; the budget counts plants once (it counted the overlap copies three times, thinning the jungle to 42 % of its authored density) |
| Sea bed near the shore has no pebbles, shells, stones, sponges, starfish; almost all bare everywhere; nothing at the three reef sites | two causes: the layer was not drawing (above); the densities were invisible even drawn — pebble mats 25/ha at the shoreline (one per 400 m²), shells 18/ha of 4–15 cm props, coral 15+15/ha; within 60 m of the beach spawn the old bundles held 6 pebble mats and within 60 m of the "densest" reef cell no coral at all | `SEABED_BAND` re-authored for a swimmer's sight line: pebbles 120, shells 70, barnacles 12, coral 40+40, starfish 16, sponges 24, driftwood 3, algae 45 /ha at the shoreline on the same ramp; the ring's bed covers tripled (cover 0: 2,400 + 2,100 /ha; 33: 1,200/1,200/750; 35: 900/900/750). Census after the run (725,609 instances, +45k): coral 3,873 + spiky 4,408, starfish 2,018, sponges 3,340, pebble mats 31,651, algae mats 10,274; within 60 m of the beach spawn 411 scatter pieces (264 algae mats, 42 + 20 pebble mats, 27 clams, 10 snail shells, scallops) plus ~2,200 ring pieces; within 60 m of the reef cell 184 (85 + 42 pebble mats, 23 spiky coral, 10 sponges, 7 clams). The other reef sites keep their coordinates (`x=5.33&z=4.88`, `x=5.62&z=2.32`) |
...
| A floating, open-backed rock at 1.59 E / 2.30 S | `rock_census` passes it (plane 1.35 m under); by its mesh the pile's side boulder stands 1.28 m off the ground. Seating every rock by its underside was tried, sank nearly all of them too deep and refused a quarter; the owner reverted it the same day | round-4 seating restored exactly (13 plane-floats, 27 swallowed, 0 open backs out); `cut_hanging_rocks` drops boulders and piles whose lower half hangs past 0.75 m at the encoded pose (0075 §7): 792 of 18,517 cut, 17,725 stand, the pile among the cut. Cliff shells untouched |

**Cut (owner, 2026-09-23): road-wear ground paint.** Road condition never painted the ground; the 2026-09-16 wear bake never ran and its synthetic bake reached 17 % where 80 % was expected; owner cut the paint 2026-09-23, condition stays a route fact; may return as a polish-pass local patch. Code: the condition branches in `landcover.compile_ground_control` removed (every major road paints alike, as the shipped control map does); `test_road_condition.py` lost its two paint tests and `test_landcover.py` its `test_built_surface_recedes_as_the_road_decays`; the scatter's condition thinning and the route records' `condition` stay.
