...
**3. Waiting turns.** In the three most recent transcripts (excluding this one): 14 of 96 planner turns in `7ecb8df9` and 8 of 236 in `4157f1e4` were pure "still waiting on the agents" replies with no tool call, at 180–190k context each. In the first that is ~2.5M cached tokens, ~15% of that session's planner bill. Mechanism (owner's observation, confirmed in this session): each subagent wakes the planner **twice**, once with its hand-back message and again with the completion notification; the planner replies to both. Three agents launched together produced six wakes, four of which did nothing. The whole conversation is re-read on every wake, so a shorter reply saves nothing; only fewer wakes do. The second wake comes from Claude Code itself; the planner has no way to stop it. What it can do: (a) never spend a turn whose only content is an acknowledgement or a status line; (b) brief agents so each report is actionable alone and act on it when it lands; (c) put several small look-ups in one brief rather than one agent each; (d) for a fan-out of three or more agents with nothing to do between them, use one `Workflow` script, which notifies once. The Workflow tool requires explicit owner opt-in per its own rules. The CLAUDE.md golden rule "frequent, short progress updates" is part of the cause: it reads as a licence to spend a turn on an update.

**4. "Minimise turns" as an instruction.** A blanket rule would be counter-productive: the planner would batch into giant turns, do shell work itself to avoid a delegation round-trip, then skip required check-ins. Name the specific wasteful turn types instead: acknowledgement-only turns; independent tool calls issued one per turn; gate fix cycles run by the planner.
...
Owner (2026-09-21): 1 approved (golden rule reworded). 2 approved with 3 taken further: docs are exempt from the prose linter entirely (cosmetic), it runs over player-facing text only, and its reds are fixed by the `run` agent. 4 approved in spirit, hard cap rejected: a specific report-shape section in each agent definition instead. 5 approved (opt-in line in CLAUDE.md). Recorded as 0079 addendum rules 11–13. Next review measures: waiting turns per agent-heavy session, Agent share of carried context, turns spent after the first gate run.
