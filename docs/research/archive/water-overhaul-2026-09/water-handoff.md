...
- Verified live checkpoint: `4e0f841`, Actions `34134844669`, bundle
  `index-St7ayzUO.js` (2026-09-07). Tests, typecheck, credits, build and deploy
  pass. Downloaded bundle SHA256:
  `b6340068538070132f886c2ffd67e27f4de60fe75336133c6c14efe022b86781`.
...
## Ocean rendering correction deployed (2026-09-07)

Live4e0f841 verified after successful Actions34134844669; root tests/typecheck
pass. Public bundle contains both corrections; preview metadata is unchanged
(`/tmp/water-pages-34134844669-live.json`).
...
## Next waterfall implementation boundary

Independent source review: v2 has4,017 ribbons/zero falling intervals; preview
has16,394 ribbons/729 falling intervals. Both have256 decorative cascade
records. Neither has a continuous closed ballistic water volume. Current
`water_features.py` labels a reach falling from a steep bed slope, which does
not establish terrain detachment; supported rapids must remain surface ribbons.
`fallingSpray.ts` infers launch velocity backwards from a designated plunge.
Next implementation must verify terrain departure, obtain incoming flow, trace
its gravity arc to the actual receiving water/terrain, and share that trajectory
between closed water sections, finite contact, spray and impact mist. Section
thickness follows discharge/width/speed. Do not make the air beneath swimmable.
Backlog threads21564/55541 could not be retrieved by the reviewer; no tutorial
contents were used. This is a source audit and implementation requirement,
not completed waterfall work.
