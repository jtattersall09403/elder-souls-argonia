...
|---|---|---|
| Chain hygiene (16b, found closing 16c 2026-09-14): `compile_hydrology` no longer reproduces the pass on which the shaped ground was frozen (3 minor river cells: `hydrology-meta.json` riverCells.minor 5950 → 5953, riverCellsInSeaLevelWater 2008 → 2011), so `shape_province` refuses with sha `7b98d33a…` against the frozen `4d81cdf8…` and the chain only runs with `--from`; the vault's `hydrology-pass1.npz` now holds the drifted pass. Root cause is a code or input edit after the 2026-09-13 corrections run — diff `compile_hydrology`'s closure and `approved-routing-corrections.json` against commit 69ce565d | research/phase16/16c-water-once-ledger.md §7 | a plain `terrain-chain.sh` run skips every frozen stage and reproduces the frozen shas; never `--refreeze` to make it so |
