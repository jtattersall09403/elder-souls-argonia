...
| Kit rebuild: identical decimation levels still ship (86 of 159 land assets, all of the underwater kit's duplicates; the runtime drops them, `build_kit.py` no longer exports them) and the underwater kit's 9 rocks carry decimated levels against 0071 (`lodRatiosByCategory rock: []` missing from its config). `python -m pipeline.kit_lod_audit` reports both | 16f round 5 (0075 §3–4) | rebuild + recompress both kits at the next kit build; bytes only, no visible change |
