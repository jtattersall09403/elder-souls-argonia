...
| Kit rebuild: decimated levels still ship for alpha-tested assets (unused: the runtime substitutes the base geometry; `build_kit.py` no longer exports them; identical duplicates likewise) and the underwater kit's 9 rocks carry decimated levels against 0071 (`lodRatiosByCategory rock: []` missing from its config). `python -m pipeline.kit_lod_audit` reports both | 16f round 5 (0075 §3–4) | rebuild + recompress both kits at the next kit build; bytes only, no visible change |
| Chain order gate: eight `warn: order:` lines — stages that read an artefact a LATER stage in the same run rewrites, so they read the previous run's accepted output (`worldgen/chain_contracts.py` `WRITES` / `stale_ok`). `apply_terrain_patches` ← `province/route-structures.json` (16h turns pads and windows into patches); `solve_major_routes` ← `refined/meta.json` and ← `routes/registry.json`; `grade_routes` ← `routes-minor.json` and ← `routes/route-structures.json`; `derive_crossings` ← the catalogue; `author_route_structures` ← `travel-services.json`; `rederive_blueprints` ← `province/places.json` (16h) | 16g deliverable 0b, 2026-09-19 | each edge closed when its owning stage is next touched (16h for the settlement one); `./scripts/terrain-chain.sh --check-contracts` prints no `warn: order:` line |
...
| `test_road_condition` is red ×2 (pre-existing since the 16f bake inversion): the fixture encodes the pre-inversion graded bake, so the road-condition test measures the old ground. Mechanism: re-derive the fixture from the current graded bake. File: `tooling/world-generation/worldgen/test_road_condition.py` | found by 16g, 2026-09-19 | both tests green against the current graded bake, with no change to the road-condition rule itself |
...
| `worldgen/test_travel_services.py::test_the_crossing_list_is_current` is red: `world/sources/routes/water-crossings.json` is stale against the current record. Mechanism: `python3 -m worldgen.derive_crossings`, then commit the rewritten file — not done by the vegetation lane because it rewrites world data another lane owns | vegetation lane preflight, 2026-09-21 | 16g/16h lane: the crossing list is re-derived and committed, and the test is green |
| Site: `apps/stats-lab` (stats-lab lane, 0088) is not in the Pages site; `tooling/pages-site/compose.mjs` is 16h's file while 16h runs. Add the lab (built with `base: "./"`, so any subpath works) as one more composed app | stats-lab lane round 3, 2026-09-24 | the deployed site links the lab and `npm run site:compose` builds it |


`worldgen/test_blueprint.py::test_live_dir_validates` is also red, but it is already accounted for in the known-red register (`tooling/world-generation/worldgen/known_red.py:45`) and queued to 16i; no backlog row is needed.
