...
- **Vegetation: occlusion culling and a far tier for the ground ring** (16f close, 2026-09-16). The renderer culls by frustum (per quarter) and by per-species draw distance only; nothing behind a hill or a wall is skipped, which is what shipped worlds add next (a hierarchical depth buffer test per instance block, or the BVH culling in `@three.ez/instanced-mesh` the rendering research names). The ground-cover ring draws its 100–128-triangle ferns to the full band (0.93 M triangles at the jungle site); a cross-quad or single-card tier past 30 m would halve it. Both are measured levers, not guesses: the probe `apps/world-studio/scripts/probe-local-16f.mjs` prints the numbers.
- **Underwater band: a 4 m kelp scattered into 1 m of water** — FIXED IN CODE 2026-09-17, awaiting one `compile_scatter` run. The renderer was never at fault: instrumenting the pivot-on-terrain branch of `apps/world-studio/src/vegetation/Vegetation.tsx` (line 462) at `x=0.15&z=6.35` and `x=3.15&z=6.75` showed every underwater instance seated on the streamed seabed with its baked sink, zero null samples. The pieces were simply scattered into water shallower than themselves: `bmv:landscape/grass/waterkelptall01` and `bmv:landscape/plants/kelptallstatic01aaa` are 3.98 m tall (`sizeM[2]`) under a `water_depth_m` band starting at 0.8 m, so 2-3 m of each plant stood proud of the sea. Root cause fixed at the source rather than by re-authoring the numbers: `compile_scatter.floor_submerged_depths` raises every SUBMERGED layer's minimum depth to the piece's own drawn height at its largest scale, over the eight roles listed in `SUBMERGED_ROLES`. The band slides down-to-up rather than being clipped, so the authored thickness survives: clipping alone left coral on 20 ha of the whole province against 66 ha once its 4 m thickness is kept. Reeds, lilypads, drowned trees and drowned thickets are excluded by name, because all four stand proud of the water by design. 264 shipped layers move. **Measured cost, worth a follow-up:** the floor uses the full box height `sizeM[2]`, which over-states a piece whose pivot sits above its base, so small objects lost more shallow ground than they should. On the 2026-09-17 run the shell beds fell from 985 pieces to 31 and the wreckage from roughly 30 to 16, because both are authored into the shore strip by their `land_cover` gate and a 0.5 m clam no longer fits 0.4 m of water. The exact quantity wanted is the height a piece shows ABOVE the bed, which is `sizeM[2]` minus the part below the pivot minus the designed sink. `originOffsetM` cannot be used for that yet: `apps/world-studio/src/vegetation/floraKit.ts:51` documents it as origin-to-bbox-max while line 98 reads it as `originAboveBase`. `waterkelptall01` carries 0.09 against a 3.98 m box, which fits neither reading. Pin the kit's pivot convention down first, then refine the floor; guessing it would put the 4 m kelp back above the sea. Shell beds are naturally intertidal, so their `land_cover` gate may be the better lever. REMAINING: the palette is read at scatter time, so the fix is inert until `terrain-chain.sh --from compile_scatter` runs (57 s over 256 chunks). The recipe hash already carries the change, so the next scatter re-does the whole province by itself. Publish the rasters after it, since the bundles ship in the rolling release rather than in git.
