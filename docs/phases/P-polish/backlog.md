...
- **Underwater band: kelp re-grounded above the waterline** (16f owner walk, 2026-09-17). The scatter places the band at the record's bed depth, but `apps/world-studio/src/vegetation/Vegetation.tsx` (about line 455, `groundHeightM`) re-grounds every pivot-on-terrain species to the STREAMED terrain, so where the rendered ground sits above the record's depth the kelp stands in air (measured at `x=0.15&z=6.35`: `waterkelptall01` and `kelptallstatic01aaa` with bounding centres at y = +0.5 m, 17 instances). Mechanism: give the underwater kit's species the ground ring's `waterRule: below-at-least` gate in the renderer (Groundcover.tsx already samples the water level for it) so an instance whose re-grounded pivot is above the water level is dropped; count dropped per species in `VegetationStats.bySpecies` (added 2026-09-17). Not a draw-distance defect: the band's 60–139 m draw distance is at or beyond underwater visibility, and the band is drawn and painting pixels at that site (probe, 1,657 instances, 338 kelp).
