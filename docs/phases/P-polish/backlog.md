...
- **Underwater band: a 4 m kelp scattered into 1 m of water** (16f owner walk 2026-09-17; re-measured 2026-09-17, the earlier "re-grounded above the waterline" reading was wrong). The renderer is not at fault. Instrumenting the pivot-on-terrain branch of `apps/world-studio/src/vegetation/Vegetation.tsx` (line 462, `groundHeightM`) at `x=0.15&z=6.35` and `x=3.15&z=6.75` showed every underwater instance seated on the streamed seabed with its baked sink, zero null samples, the sampled ground within 0.05 m of the baked height on flat bed (0.9 m on one bank). The two reported species come from the LAND palette layer `aquatic-kelp` in `world/sources/flora/palettes.json`, whose `water_depth_m` band is [0.8, 6.0] with a 2.0 m peak, while the meshes `bmv:landscape/grass/waterkelptall01` and `bmv:landscape/plants/kelptallstatic01aaa` are 3.98 m tall (`sizeM[2]`, flora-province-v1 kit). At the measured sites they sit correctly on bed at y -0.86 to -1.73, so 2-3 m of each plant stands proud of the sea surface. The deep-band species at the same sites sit at y -3.5 to -11.8, inside their own band. Mechanism: raise that layer's minimum water depth above the species' own drawn height (or move these two ids to a shallow-water species that is under a metre tall), then re-scatter. That is a palette decision plus a scatter re-run, so the call belongs to Fable rather than to a renderer patch.
