...
- **Place names and quest titles are raw strings in `world/sources/catalogue` and `world/sources/quests`, rendered without text-catalogue IDs (standard 2).** A generator must lift them into `packages/text-catalogue/src/generated/` so they are linted at the funnel and translatable. Evidence: find-agent trace 2026-09-21; no such generator exists (`tooling/world-generation/worldgen/` has `travel_services.py` and the hydrology-names generator only). Done when every player-visible name resolves through a text-catalogue id and `lint_prose --strict` covers it.
