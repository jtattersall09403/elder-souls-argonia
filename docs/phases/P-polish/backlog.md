...
- **The importer cannot read `hkaInterleavedUncompressedAnimation` (weapons lane round 1, decision 0077 §5a).** PyNifly reads only `hkaSplineCompressedAnimation`; eighteen Animated Armoury clips, including the whole drawn-katana set (`_CustomConditions/19` and `/20`), are interleaved. The class is a per-frame array of `hkQsTransform` per track, simpler than the spline path. Fix: add the reader beside the spline one in the pipeline's HKX import, prove it on `20/1hm_attackright.hkx` against the rig, then build the `katana` pack per the lane brief and retire the katana class's `borrowedMoveset`.
- **Three source mods held on unverified permissions (decision 0077 context).** Animated Heavy Armory (SSE 51100), Skyrim Spear Mechanic (SSE 25146) and Black Marsh Import (LE 48551) are downloaded and hashed in the vault but ship nothing: the Nexus API omits the permission box and the pages return 403 from the VM. Owner reads the three pages; a "yes" opens the AHA shortspear/trident meshes, the Spear Mechanic one-handed thrust set and the Black Marsh OBJ weapons (which also need an OBJ path in `build_weapons.py`).
