...
- **Underwater band: a 4 m kelp scattered into 1 m of water** — FIXED IN CODE 2026-09-17, awaiting one `compile_scatter` run. The renderer was never at fault: instrumenting the pivot-on-terrain branch of `apps/world-studio/src/vegetation/Vegetation.tsx` (line 462) at `x=0.15&z=6.35` and `x=3.15&z=6.75` showed every underwater instance seated on the streamed seabed with its baked sink, zero null samples. The pieces were simply scattered into water shallower than themselves: `bmv:landscape/grass/waterkelptall01` and `bmv:landscape/plants/kelptallstatic01aaa` are 3.98 m tall (`sizeM[2]`) under a `water_depth_m` band starting at 0.8 m, so 2-3 m of each plant stood proud of the sea. Root cause fixed at the source rather than by re-authoring the numbers: `compile_scatter.floor_submerged_depths` raises every SUBMERGED layer's minimum depth to the piece's own drawn height at its largest scale, over the eight roles listed in `SUBMERGED_ROLES`. Reeds, lilypads, drowned trees and drowned thickets are excluded by name, because all four stand proud of the water by design. 264 shipped layers are floored; the tall kelp keeps 87 % of the ocean (1,511 ha of 1,742 ha is deeper than its 5.18 m floor), so only the shallow fringe is lost. REMAINING: the palette is read at scatter time, so the fix is inert until `terrain-chain.sh --from compile_scatter` runs (57 s over 256 chunks). The recipe hash already carries the change, so the next scatter re-does the whole province by itself. Publish the rasters after it, since the bundles ship in the rolling release rather than in git.
