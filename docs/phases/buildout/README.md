...
> **To be reviewed (owner 2026-09-18, decision 0074 §5).** Parts of this
> register are stale — it counts eleven engineering standards (there are
...
- **Minimal NPC detection model** (view cones + seen/unseen, one service). **RATIFIED 2026-09-13: Phase 10b deliverable** (phases README).
...
- **By 10c — RATIFIED 2026-09-13 as 10c deliverables** (phases README): magic `StatEffect.field` enum + cast-interruption-vs-poise
...
| **Dialogue/interaction UI** | 16e ships a minimal talk-to-service contract (ferries, merchants); persuasion math (76 §125) | topic web, KnowledgeState, glossary + newcomer-topic coverage (Q0 gate), disposition verbs + the refusal ladder (cross-check §4) | Phase 11 ships talk→service-menu as a small *contract*, not a ferry hack |
...
| **Combat verb completion** | 10b wires the sourced movesets (pull-in); poise/equip-load at 10c | charged/running/jumping/plunging attacks, kick/shield bash, two-handing, weapon poisons, H2H fatigue-takedown finisher (0039: Morrowind shape confirmed) — **killmove layer, projectile interception and bleedout states all CUT by 0039 rulings** | attack taxonomy stays data on the class table (76 §121.3) **Hook in hand (0074): the weapons lane ships the per-class `effects` slot and the single damage-resolve step; new verbs extend the enum, never the pipeline.** |
...
| **Hist communion powers** (0042 §1, KEPT) | 16g places the ~10 hero Hist with a stable ID and a power slot | the powers themselves as effect-stack data (once-per-day semantics), plus the flavour/dream layer | race-neutral: anyone may drink; Argonians get flavour and a dream, never mechanics, and nothing is gated on race |
