...
/** The square of built ground (16d): ONE name for this number, the contract's.
 * The old derivation `(SOURCE_GRID_SAMPLES - 1) * RAW_METRES_PER_SAMPLE` is
 * asserted below, so a change to the sample grid cannot silently leave the
 * boundary wall behind. */
export const TERRAIN_SUPPORT_EXTENT_M = PROVINCE_BOUNDARY.extentM;
if (Math.abs((SOURCE_GRID_SAMPLES - 1) * RAW_METRES_PER_SAMPLE - TERRAIN_SUPPORT_EXTENT_M) > 1e-6) {
  throw new Error("PROVINCE_BOUNDARY.extentM no longer matches the refined sample grid");
}
