...
| Action | Where | Notes |
| --- | --- | --- |
| Free-roam walk/run top speed | `PLAYER_WALK_SPEED` in [`packages/game-core/src/io/input.ts`](../../../../packages/game-core/src/io/input.ts) | Also drives `<Ecctrl maxWalkVel>` in `CombatRuntime.tsx` (`packages/character/src/combat/`) — same constant, single source of truth |
| Free-roam sprint top speed | `PLAYER_SPRINT_SPEED` in [`packages/game-core/src/io/input.ts`](../../../../packages/game-core/src/io/input.ts) | Also drives `<Ecctrl maxRunVel>` in `CombatRuntime.tsx` (`packages/character/src/combat/`). Sprint is disabled while locked on (`lockOnSprintAllowed`) |
...
| Locked-on walk/strafe top speed, *switch off* | `PLAYER_LOCK_ON_WALK_SPEED` in [`packages/game-core/src/io/input.ts`](../../../../packages/game-core/src/io/input.ts) | Only used when "Locked-on speed follows the strafe clips" is off; derives the `lockOnMoveScale` applied to joystick input in `CombatRuntime.tsx` (`packages/character/src/combat/`) |
| Enemy walk/run speed | `ENEMY_LOCOMOTION.walkVel` / `.runVel` in [`src/game/combat/tuning.ts`](../../../../packages/game-core/src/combat/tuning.ts) | Also drives the enemy `<Ecctrl>` speed props directly |
| Roll launch + decay speed | `DODGE_SPEED.playerRoll` in `tuning.ts`, consumed in `CombatRuntime.tsx` (`packages/character/src/combat/`)'s `dodgeReleased` handler (launch) and the `playerAction.current === "roll"` decay block | Same constant used in both places; duration is `COMBAT_TUNING.rollDuration` (`weapon.ts`) — leave that alone to keep clip timing unchanged |
| Backstep launch + decay speed | `DODGE_SPEED.playerBackstep` in `tuning.ts`, consumed the same way in `CombatRuntime.tsx` (`packages/character/src/combat/`) | Duration is `ACTION_DURATIONS.backstep` (`tuning.ts`) |
| Enemy roll/backstep speed | `DODGE_SPEED.enemyRoll` / `.enemyBackstep` in `tuning.ts` | Consumed in `CombatRuntime.tsx` (`packages/character/src/combat/`)'s enemy `dodge`/`backstep` state handling |
...
(see `action.timeScale = ...` in [`SkyrimFighter.tsx`](../../../../packages/character/src/SkyrimFighter.tsx)).
`speedMultiplierRef` is `playerAnimationSpeed` in `CombatRuntime.tsx` (`packages/character/src/combat/`) — for
...
`moveMagnitude` in `CombatRuntime.tsx` (`packages/character/src/combat/`) is a genuine 0..1. Both the free-roam
