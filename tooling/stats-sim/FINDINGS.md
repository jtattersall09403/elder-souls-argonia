...
| 39 | Poise reinstated (owner reversal of round 2) on the DS1 model, with base poise = Agility/2 replacing the bare damage-threshold stagger rule. | **Design only for now** — the harness's fight model is a damage race that never models stagger, so poise adds no simulated numbers yet. Constants are provisional until sandbox calibration at 10c (research: `docs/research/combat-and-systems/dark-souls-poise-mechanics.md`). |
