    ...
    for layer in palette.layers:
        ...
        for cell_z in range(gz0, gz1 + 1):
            for cell_x in range(gx0, gx1 + 1):
                key = hash64(salt, cell_x & 0xFFFFFFFF, cell_z & 0xFFFFFFFF)
                if uniform_at(key, 0) > min(1.0, per_cell):
                    continue
                ...
                count = 1 if singles else _clump_size(
                    uniform_at(key, 5), layer.clump_size_median, layer.clump_size_tail)
                ...
                for member in range(count):
                    ...
                    if member == 0:
                        ...
                        # sqrt keeps members uniform over the disc rather than
                        # piling them on the centre.
                        radius = layer.clump_radius_m * math.sqrt(uniform_at(mkey, 1))
                    ...
                    yaw = uniform_at(mkey, 4) * math.tau if layer.yaw_random else 0.0
                    ...
                    if layer.clearance_radius_m > 0:
                        stamps.append((px, pz, layer.clearance_radius_m * scale))
    return instances
