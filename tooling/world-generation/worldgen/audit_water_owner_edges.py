    ...
    for ribbon in meta['ribbons']:
        ...
        for i, point in enumerate(points):
            ...
            for side, kind in enumerate(point['boundaryKinds']):
                ...
                if sample[2] > 0 or point['y'] - sample[1] <= .004:
                    continue
                ...
                cases.append({'id': ribbon['id'], 'point': i, 'side': side, 'height': point['y'],
                    'inside': inside.tolist(), 'outside': outside.tolist(), 'raster': raster(*outside)})
