...
def build_report() -> dict:
    ...
    # dry ground read through the record reader (decisions 0049, 0066): the
    # dry-season signed depth the water bake publishes, never the class
    # raster. `water-class.png` is a TYPE label drawn over a wider area than
    # the water itself; reading land off it dropped walkable ground from the
    # denominator and inflated every band's rate. Nothing here reads it any
    # more, and the gap it used to cause is measured below rather than quoted
    # so it cannot go stale after a bake.
    land = s.dry_grid                              # dry-season signed depth <= 0
    measured = {
        "measuredLandKm2": round(float(land.sum()) * cell_km2, 2),
        "measuredStandingWaterKm2": round(float(s.wet_grid.sum()) * cell_km2, 2),
        "wetSeasonStandingWaterKm2": round(float(s.wet_season_grid.sum()) * cell_km2, 2),
    }
    bands = {}
    for b in range(1, 6):
    ...
    return {"morrowind": {"hostilePerKm2": MORROWIND_PER_KM2, "spacingM": MORROWIND_SPACING_M},
            # measured off the shipped water through the record reader, never
            # quoted from a past bake
            "landMeasured": measured,
...
def render(rep: dict) -> str:
    ...
    L = ["# Hostility frequency by danger band", "",
         ...
         f"Land is MEASURED dry-season dry ground, read through the record reader "
         f"(`ShippedWater.wet_grid(\"dry\")`): every cell the water bake publishes at or below "
         f"zero signed depth in the dry season, which comes to {ce['measuredLandKm2']} km². "
         f"It is never the water class raster, a type label drawn over a wider "
         f"area than the water itself: reading land off it dropped walkable ground from "
         f"the denominator and raised every band's rate to match (decisions 0049, 0066).", "",
