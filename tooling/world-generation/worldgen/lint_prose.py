...
SCOPE (owner, 2026-09-21): the linter lints by EXCLUSION. `lint_roots()` walks
world/sources/ and packages/text-catalogue/ and lints every prose-looking
string (>=6 words, or sentence punctuation with >=3), so a new kind of text is
linted the day it exists; the only way to skip one is a key or directory in
EXEMPT_KEYS / EXEMPT_DIRS / EXEMPT_FILES, each with its reason. `--tripwire`
fails on prose data outside those roots. The field lists below stay as the
RECORD-AWARE surfaces (per-record rules and density scopes), not as the scope.
The record-aware run covers PLAYER-VISIBLE and WORLD-RECORD text — prose FIELDS of live catalogue records, the quest rows, the text
...
# ---------------------------------------------------------------------------
# The field-inclusion lists above are the *record-aware* surfaces (they carry
# per-record rules: hook-copies-site, generaliser caps, field echo). They are
# no longer what DEFINES the scope. The scope is now every prose-looking string
# under the linted roots; a new kind of player-facing text (dialogue, item
# descriptions, book text, signs) is linted the moment it exists, and the only
# way to skip a string is a key or a directory listed below WITH its reason.
...
# Key names that are never prose. A value under one of these is an identifier,
# a path, a machine enum or a classification, whatever its length.
EXEMPT_KEYS = {
    "id", "ref", "refs", "path", "file", "hash", "url", "source", "kind",
    "type", "schemaVersion", "tags", "sha256", "sha", "licence", "license",
...
EXEMPT_DIRS = {
    ...
    "world/sources/sites/dossiers":
        "per-site working dossiers generated by the placement tools from the "
        "catalogue records that are themselves linted above",
}
# Individual files under the roots that are machine output or quoted source.
EXEMPT_FILES = {
    ...
    "world/sources/sites/plot-remedies.json":
        "generated remedy rows written by the plot tooling from the catalogue "
        "records, which are linted",
    "world/sources/sites/semantic-audit.md":
        "generated audit output quoting the records it audits",
    "world/sources/sites/prose-lint.md":
        "this linter's own generated report — it quotes the hits it found",
    "world/sources/sites/hostility-frequency.md":
        "generated frequency table from the catalogue records, which are linted",
...
def looks_like_prose(text: str) -> bool:
    """≥6 words, or sentence punctuation with ≥3 words (owner 2026-09-21)."""
    ...
    if n >= 6:
        return True
    return n >= 3 and bool(_SENTENCE_PUNCT.search(text))


def is_exempt_key(key: str) -> bool:
    return key in EXEMPT_KEYS or key.endswith(EXEMPT_KEY_SUFFIXES)


def _exempt_dir_reason(rel: str) -> str | None:
...
# rule the catalogue lint already applies (lint_catalogue skips these), applied
# to every record shape under the roots.
DEAD_STATUS = {"cut", "deferred"}


def iter_prose_strings(node, pointer: str = ""):
    """Yield (json-pointer, string) for every prose-looking value in `node`."""
    if isinstance(node, dict):
        if node.get("status") in DEAD_STATUS:
            return
        for k, v in node.items():
            if isinstance(k, str) and is_exempt_key(k):
                continue
            yield from iter_prose_strings(v, f"{pointer}/{k}")
    elif isinstance(node, list):
        for i, v in enumerate(node):
            yield from iter_prose_strings(v, f"{pointer}/{i}")
    elif isinstance(node, str) and looks_like_prose(node):
        yield pointer or "/", node


def _load_data(path: Path):
    """Parse a data file into python objects (a jsonl file yields a list)."""
    text = path.read_text(encoding="utf-8")
    if path.suffix == ".jsonl":
        return [json.loads(ln) for ln in text.splitlines() if ln.strip()]
    if path.suffix in (".yaml", ".yml"):
        import yaml                      # optional: no YAML exists under the
        return list(yaml.safe_load_all(text))   # roots today, but a new one lints
...
def lint_source_file(res: LintResult, path: Path, rel: str) -> None:
    """Every prose-looking string literal in a .ts/.tsx file under the roots."""
    for n, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        if line.lstrip().startswith(("//", "*", "/*")):
            continue
        for m in _TS_STRING.finditer(line):
            ...
            if looks_like_prose(s):
                res.add_text(rel, f"{rel}:{n}", "string", s)


def lint_roots(res: LintResult) -> list[str]:
    """Walk the linted roots and lint every prose-looking string.

    Returns the list of files skipped, each with its reason — the report shows
    them, so an exemption is visible rather than silent.
    """
    skipped: list[str] = []
    for root in LINT_ROOTS:
        base = catalogue.REPO_ROOT / root
        if not base.exists():
            continue
        for path in sorted(base.rglob("*")):
            ...
            if suffix not in DATA_SUFFIXES | TEXT_SUFFIXES | CODE_SUFFIXES:
                continue
            ...
            try:
                ...
                raise RuntimeError(f"{rel} is not readable as data: {exc}") from exc
            for pointer, text in iter_prose_strings(data):
                res.add_text(rel, rel, pointer, text)
    return skipped


# Data files outside the roots that are NOT player text. Same rule as the
# exemptions above: a row here needs a reason, and there is no other way to
# skip a file.
TRIPWIRE_EXEMPT = {
    "package.json":
        "npm package metadata: the `description` field is for the registry, "
        "never rendered in the game",
    "packages/game-core/src/validation/visualAnimationExclusions.json":
        "test rationale — why each animation is excluded from the visual "
        "check; read by agents and the gate, never by a player",
    "packages/game-core/src/validation/visualScenarioExpectations.json":
        "test rationale for the visual scenarios, same class as the above",
}


TRIPWIRE_ROOTS = ("packages", "apps")
TRIPWIRE_SUFFIXES = {".json", ".jsonl", ".yaml", ".yml"}
TRIPWIRE_MIN_WORDS = 12


def tripwire_hits() -> list[tuple[str, str, str]]:
    """Prose data OUTSIDE the linted roots: (file, pointer, excerpt).

    A ≥12-word sentence in a data file under packages/ or apps/*/src/ is player
    text or a world record living where nothing lints it.
    """
    out: list[tuple[str, str, str]] = []
    roots = [catalogue.REPO_ROOT / "packages"]
    apps = catalogue.REPO_ROOT / "apps"
    if apps.exists():
        roots += [p / "src" for p in sorted(apps.iterdir()) if (p / "src").is_dir()]
    for base in roots:
        if not base.exists():
            continue
        for path in sorted(base.rglob("*")):
            if not path.is_file() or path.suffix.lower() not in TRIPWIRE_SUFFIXES:
                continue
            rel = str(path.relative_to(catalogue.REPO_ROOT))
            marked = f"/{rel}"
            if any(x in marked for x in ("/node_modules/", "/dist/", "/generated/",
                                         "/__fixtures__/", "/text-catalogue/")):
                continue
            if ".test." in path.name:
                continue
            if rel in TRIPWIRE_EXEMPT or path.name in TRIPWIRE_EXEMPT:
                continue
            try:
                data = _load_data(path)
            except Exception:
                continue
            for pointer, text in iter_prose_strings(data):
                if len(text.split()) >= TRIPWIRE_MIN_WORDS and _SENTENCE_PUNCT.search(text):
                    out.append((rel, pointer, text[:90]))
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--roots", action="store_true",
                    help="walk the linted roots and lint every prose-looking string (lint by exclusion, owner 2026-09-21)")
    ap.add_argument("--tripwire", action="store_true",
                    help="report prose data OUTSIDE the linted roots and exit 1 if any (the repo-standards tripwire)")
    ...
    if a.tripwire:
        ...
        for f, ptr, ex in hits:
            print(f"{f} {ptr}: {ex}")
        if hits:
            print(f"{len(hits)} prose strings in data outside the linted roots")
