...
def main(argv: list[str] | None = None) -> int:
    ...
    if not a.catalogue and not a.md:
        return run_gate(a.strict)

    if a.report is None:
        a.report = Path("-") if a.region else REPORT_PATH
    res = lint_catalogue(set(a.region) if a.region else None) if a.catalogue else LintResult()
    if a.catalogue and not a.region:
        lint_quests(res)
    ...
    title = "world records" + (f" ({', '.join(a.region)})" if a.region else "")
    if md_paths:
    ...
    if not a.quiet:
        ...
        for s_, r_, d_ in fails:
            print(f"  density {s_} {r_} = {d_}/1k words (max {GLOBAL_DENSITY_MAX[r_] if s_ == 'WHOLE RUN' else SOFT_DENSITY_MAX[r_]})")
        if str(a.report) != "-":
