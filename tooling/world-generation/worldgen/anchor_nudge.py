...
# The graph's vocabulary (hydrology-graph.json `vocabulary`): a reach kind is a
# river; a standing body kind that is not marsh is a lake or pond (0066).
RIVER_KINDS = frozenset({"horizontal-channel", "horizontal-tidal", "horizontal-backwater",
                         "sloped-riffle", "sloped-rapid", "sloped-chute", "vertical-fall"})
LAKE_KINDS = frozenset({"lagoon", "lake-lowland", "tarn-upland", "pond", "pool", "plunge-pool"})


def water_fit(need: str, sm: dict) -> float:
    h = sm["hydrology"]
    coast, shore, kind = h["coastDistanceM"], h["shoreDistanceM"], h["record"]["kind"]
    on_river, on_lake = kind in RIVER_KINDS, kind in LAKE_KINDS
    if need == "coast":
        return 1.0 if coast <= 250 else 0.5 if coast <= 500 else 0.0
    if need == "lake":
        return 1.0 if on_lake or shore <= 120 else 0.0
    if need == "river":
        return 1.0 if on_river or shore <= 90 else 0.5 if shore <= 250 else 0.0
    if need == "river-or-coast":
        return max(1.0 if coast <= 300 else 0.0, 1.0 if on_river or shore <= 90 else 0.5 if shore <= 250 else 0.0)
    if need == "marsh":
...
def score_site(a: dict, s: ProvinceSurvey, x: float, z: float, home_zone: str | None,
    ...
    facts = {
        ...
        "waterKind": sm["hydrology"]["record"]["kind"] or "-", "region": sm["regionName"],
...
def digest(res: dict) -> str:
    ...
    cols = ["score", "moveM", "buildablePct", "landPct", "meanSlope", "dangerMax", "coastM", "shoreM", "waterKind", "region", "zone", "elev"]
