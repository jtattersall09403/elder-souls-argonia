...
import { UNIT_CLIP_TIMING } from "./clipTiming";
...
describe("mutable animation commands", () => {
  ...
  it("updates state, source time, and serial synchronously without replacing the command", () => {
    ...
    expect(command).toEqual({ state: "LIGHT_1", startAt: 0.03, crossFadeDuration: null, timing: UNIT_CLIP_TIMING, serial: 1 });
  });
  ...
  it("treats the same state as a no-op unless a restart is requested", () => {
    ...
    expect(command).toEqual({ state: "LIGHT_2", startAt: 0.1, crossFadeDuration: null, timing: UNIT_CLIP_TIMING, serial: 0 });

    expect(updateAnimationCommand(command, "LIGHT_2", 0.4, true, 0.24)).toBe(true);
    expect(command).toEqual({ state: "LIGHT_2", startAt: 0.4, crossFadeDuration: 0.24, timing: UNIT_CLIP_TIMING, serial: 1 });
  });
