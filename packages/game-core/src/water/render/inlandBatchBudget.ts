...
export function inlandBatchBudget(sources: readonly BufferGeometry[], compactNative = false) {
  ...
  const vertexBytes = (enhanced ? (compactNative ? 39 : 49) : 19) + (sources.some(source => source.hasAttribute('waterCellSize')) ? 4 : 0);
