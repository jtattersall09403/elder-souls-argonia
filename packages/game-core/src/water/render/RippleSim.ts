...
export class RippleBoundaryMask {
  ...
  update(focusX: number, focusZ: number, epoch: number, dt: number): boolean {
    ...
    if (!full && !rowCount && dx === 0 && dz === 0) return false;
    ...
    for (let z = 0; z < this.size; z++) for (let x = 0; x < this.size; x++) {
      ...
      const oldX = x + dx;
      const oldZ = z + dz;
      const refreshCell = full || (z >= rowStart && z < rowStart + rowCount);
      ...
      let label = 0;
      if (id && Number.isFinite(depth) && depth > 0.025) {
        ...
        for (let cornerZ = z; cornerZ <= z + 1 && label; cornerZ++) {
          for (let cornerX = x; cornerX <= x + 1; cornerX++) {
            ...
            if (this.cornerLabels[corner] === 65535) {
              ...
              this.cornerLabels[corner] = edge.waterBodyId && Number.isFinite(edge.depth) && edge.depth > 0.025 ? this.bodyLabel(edge.waterBodyId) : 0;
            }
    ...
    else { this.rowsRemaining -= rowCount; this.refreshRow += rowCount; }
    return true;
  }
...
export class RippleSim {
  ...
  configureBoundary(query: WorldWaterQuery, epochMinutes: () => number): void {
    ...
    const cheap = query as WorldWaterQuery & { sampleBoundary?: RippleBoundarySampler };
    this.mask.setSampler(this.fastSampler ?? (cheap.sampleBoundary
      ? (x, z, epoch) => cheap.sampleBoundary!(x, z, epoch)
      : (x, z, epoch) => query.sample({ x, y: 0, z }, epoch)));
  }

  /** Explicitly refresh after changed terrain/support; normal levels refresh at 5 Hz. */
  invalidateBoundary(): void { this.mask.invalidate(); }
  ...
  step(renderer: THREE.WebGLRenderer, focusX: number, focusZ: number, deltaS: number): void {
    ...
    const maskChanged = this.mask.update(this.center.x, this.center.y, this.epoch(), deltaS);
    if (maskChanged) { this.maskTexture.image.data = this.mask.data; this.maskTexture.needsUpdate = true; }
    for (const material of [this.copy, this.drop, this.update]) {
  ...
  dispose(): void {
    ...
    this.disposed = true;
    this.pendingDrops.length = 0;
    this.a.dispose(); this.b.dispose(); this.maskTexture.dispose();
    this.copy.dispose(); this.update.dispose(); this.drop.dispose();
