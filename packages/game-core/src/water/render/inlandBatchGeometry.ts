...
/** Copy into the final displayed layout incrementally. Source tiles remain
 * unchanged, and compact native fields retain their original Float32 values. */
export function* inlandBatchGeometry(sources: THREE.BufferGeometry[], compactNative = false): Generator<void, THREE.BufferGeometry> {
  ...
  const enhanced = compactNative || sources.some(source => source.hasAttribute('waterLevelResponse'));
  ...
  for (const source of sources) {
    ...
    if (compactNative) {
      ...
      for (let i = 0; i < source.getAttribute('position').count; i++) {
        const flag = srcLevels?.getZ(i) ?? 0;
        if (srcOverride.getW(i) !== 0 || srcNormal.getX(i) !== 0 || srcNormal.getY(i) !== 1 || srcNormal.getZ(i) !== 0
        ...
        override[output * 3] = srcOverride.getX(i);
        override[output * 3 + 1] = srcOverride.getY(i);
        override[output * 3 + 2] = srcOverride.getZ(i);
        if (levels) {
          levels[output * 2] = srcLevels?.getX(i) ?? 0;
          levels[output * 2 + 1] = srcLevels?.getY(i) ?? 0;
        }
