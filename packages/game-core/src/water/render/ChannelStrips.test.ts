...
describe("rock foam (16f)", () => {
  it("leaves a pillow upstream and a widening tail downstream, nothing beside", () => {
    const rocks = [{ x: 0, z: 0, radiusM: 1, tx: 1, tz: 0 }];
    expect(rockFoamAt(-0.5, 0, rocks)).toBeGreaterThan(0.3);      // pillow
    expect(rockFoamAt(-3, 0, rocks)).toBe(0);                      // upstream, out of reach
    expect(rockFoamAt(1, 0, rocks)).toBeGreaterThan(rockFoamAt(3, 0, rocks));  // the tail fades
    expect(rockFoamAt(3, 0, rocks)).toBeGreaterThan(0);
    expect(rockFoamAt(5, 0, rocks)).toBe(0);
    expect(rockFoamAt(1, 3, rocks)).toBe(0);                       // beside the tail
  });
  it("bakes the foam into the strip as a centre/edge pair", () => {
    const pts = [point(0, 0, "join"), point(10, 0, "steep"), point(20, 0, "steep"), point(30, 0, "join")];
    const strip = { id: "s", band: 2, points: pts } as unknown as ChannelStrip;
    const plain = buildChannelStripGeometry([strip]);
    const rocky = buildChannelStripGeometry([strip], { rocks: [{ x: 12, z: 0, radiusM: 1.5, tx: 1, tz: 0 }] });
    const a0 = plain.geometry.getAttribute("aRockFoam");
    const a1 = rocky.geometry.getAttribute("aRockFoam");
    let sum0 = 0, sum1 = 0;
    for (let i = 0; i < a0.count; i++) { sum0 += a0.getX(i); sum1 += a1.getX(i); }
    expect(sum0).toBe(0);
