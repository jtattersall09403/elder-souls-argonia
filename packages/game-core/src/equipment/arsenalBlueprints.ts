/** Unmeasured build inputs. Gameplay imports arsenal.ts, which applies baked reach. */
...
function buildWeapon(itemId: string, built: BuiltItem): ArsenalWeapon {
  ...
  const definition = defineWeapon({
    ...
    // Clips, timing and power come from the resolved moveset and class;
    // reach is applied from the baked mesh-and-motion measurements in arsenal.ts.
