...
- **One chunk or round per session, and the planner's context stays small** (decision 0079). The bill is turns × context length. Start a new session from PROGRESS.md for the next chunk; never let a session run on into the next job. A hook refuses exploratory commands (cat, grep, sed -n, find, git log/diff/show, inline python) and `sleep` from the planner session: when it fires, ask the `find` or `run` agent instead; never rephrase the command to slip past it. Shell output is filtered through `rtk` on this machine (`rtk recall <id>` for the full text).
