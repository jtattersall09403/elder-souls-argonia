...
- **Get visual feedback from the user.** Work up to the point where it would be sensible to get visual feedback from the user, then hand off to them, tell them what to do, what to check, and how to feed back, in plain english (especially non-technical language - the user is not an expert in game dev or technical concepts). Ideally deliver the whole of this phase and have me review at the end, unless there is a good reason to pause partway through and get feedback then (e.g. if something needs a steer that would be hard to change if reviewed at the end).
