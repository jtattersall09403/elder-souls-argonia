...
| `kit-assemblies-mined.json` | vanilla `Tamriel`, the three BM&V worldspace sets and HTBM Cipactli, via `worldgen/mine_assemblies.py` | **co-placement templates:** pairs and groups of pieces the source authors placed at a repeated relative offset and yaw, the pieces never placed alone, the doorways that fall out of the door-piece templates (`doorwaysFromAssemblies`, for `pipeline/interiors_index.py`), the co-placement bearings `pipeline/piece_front.py` derives a doorless piece's FRONT from (the side the authors left open), and the enclosed shells with no door from either pass. Input to the kit pipeline's COMPOSITE assets (`compose.parts`). Digest: [docs/research/placement-settlements/kit-assemblies-evidence.md](../../../docs/research/placement-settlements/kit-assemblies-evidence.md) |
| `kit-designed-sink.json` | vanilla `Skyrim.esm` plus every source-mod plugin named by `worldgen/asset_registry.py`, via `worldgen/mine_designed_sink.py` | **Phase 16h input:** per kit asset, the designed ground contact the makers used — `designedSinkM = groundZ - pivotZ` (positive = pivot below the ground line) as p25/p50/p75 over unit-scale references, the sample count, the interquartile width (the ground/elevated discriminator) and `slopeTermMPerDeg`; plus `waterline` percentiles for assets placed over water. Read onto every kit manifest by `pipeline/placement_metadata.py`. |
| `kit-mounts-mined.json` | the same plugins, exterior and interior cells, via `worldgen/mine_mounts.py` | **Phase 16h input:** `anchors` gives every kit asset its `anchorClass` and `anchorClassEvidence` (`plugin` or `mesh-tell`) under the rule in that module's docstring — placement evidence first, the mesh tell only for an asset nobody ever placed. `pairs` gives the pieces the makers hung off another piece: child, parent, offset in the parent's local z-up frame (the `kit-assemblies-mined.json` frame), relative yaw, sample count and cluster spread. A child is a wall or ceiling asset under 2.5 m and under 10 % of the parent's bounding volume, kept only at n >= 3 with a spread under 0.3 m; snap co-placements belong to `kit-assemblies-mined.json`. |
| `accepted-places.json` | written by the `place-build` slice close (step 8) | **the acceptance receipt** (0100 decision 6): per owner-accepted place, the date, the hashes of its compiled record and patches and the export's ground and kit provenance; the exporter fails when an accepted place's record would change without a `reopened` entry, and later gates run on it in report mode (`tooling/world-generation/output/accepted-report.json`) |
| `breadth-bars.json` | authored in 16k slice 1b from `building-asset-breadth.md` §3, `building-depth-and-variety.md` §5 and 0098 | **the breadth bars** the `place-build` skill reads (0099 decision 4): per-tier building and dressing numbers, enclosure kinds, distance reads and per-type pools |
...
`esp-door` entrance, which outranks every other evidence in that index's
single-`entrance` ranking), `worldgen.blueprint` (HARD rule: a linked shell is
never a mass and never stands on an unbuilt kit) and `worldgen.asset_breadth`.
