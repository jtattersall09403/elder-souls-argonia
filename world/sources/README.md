...
- [sites/](sites/README.md) — Phase 11 Part 0 site survey: the province
  **terrain scour** (candidate landform sites + the authored-land area the
  place catalogue is sized on) and per-site **dossiers**. Generated by
  `worldgen.terrain_scour` / `worldgen.site_dossier`; regenerate, never
  hand-edit.
- [registries/](registries/README.md) — the world's **id vocabularies**:
  factions, quests, creatures, NPCs, items, deed counters, rumour pools. One
  schema per file, one loader (`worldgen.registries`), and a cross-check that
